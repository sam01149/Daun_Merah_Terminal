...
// Fallback chain: FRED T-bill term structure → heuristic.
// Note: CME FedWatch/ZQ/Quote hidden APIs (CmeWS/mvc/*) removed 2026-07-24 — CME
// retired the whole endpoint family (confirmed via structured 404 on all 3, plus
// their own docs: FedWatch is now a paid EOD/Intraday API product). Yahoo Finance
// ZQ (HTTP 404 confirmed) and CME direct (IP blocked) were eliminated earlier.
...
async function computeRatePath() {
  ...
  // Step 2: heuristic fallback — distance from neutral rate (~3.0%) drives probability.
  ...
  return {
    ...
    data_note: 'Approximated from FRED DFF (CME FedWatch/ZQ discontinued as free API 2026-07-24 — see cmegroup.com/fedwatch for the paid version).',
    computed_at: new Date().toISOString(),
  };
}
...
// cacheOnly: true → JANGAN compute saat cache miss (fetch FRED bisa makan
// beberapa detik) — return { data: null }; dipakai konsumen sekunder yang
