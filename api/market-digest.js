...
// AI providers
// SambaNova (akun 1 & 2, dulu dipakai Call 1/2/3/4 sebagai fallback) — DIPUTUS
// KONTRAK TOTAL 2026-08-12: akunnya diblokir billing SambaNova sendiri ("A payment
// method is required"), ganti API key tidak memperbaikinya, lihat daun_merah_vendor.md.
...
// DeepSeek API resmi (platform.deepseek.com) — diagnostik ?test_deepseek=1 (2026-07-18,
// session 186): kandidat "primary tunggal untuk semua call" dari riset Sesi 185, BERBAYAR
// dari saldo top-up user US$2 (saldo tidak expire — yang membatasi volume, bukan waktu).
// Model deepseek-v4-flash (bukan deepseek-chat yang deprecated 2026-07-24, dan bukan
// v4-pro yang 3x lebih mahal): input $0.14/M cache-miss / $0.0028/M cache-hit, output
// $0.28/M — 1 sampel diagnostik Call 1+2+3 ≈ US$0.005. thinking disabled eksplisit
...
// providerOverride: dipakai call site yang butuh pool budget/circuit spesifik berbeda
// dari yang bisa ditebak providerFromUrl(url) (mis. beberapa akun di URL yang sama).
...
async function checkThesisContradictions(openEntries, recentItems, DEEPSEEK_KEY) {
  ...
  let raw4 = null;
  // Primary/satu-satunya: DeepSeek v4-flash (2026-08-12: SambaNova akun 1, primary
  // lama di sini TANPA fallback lain, diputus kontrak total — lihat daun_merah_vendor.md).
  if (DEEPSEEK_KEY && await cb.canCall(CB_DEEPSEEK)) {
    try {
      console.log('Call 4: trying DeepSeek v4-flash');
      raw4 = await aiCall(DEEPSEEK_URL, DEEPSEEK_KEY, DEEPSEEK_MODEL, call4Messages, 700, 0.1, 15000, {}, { thinking: { type: 'disabled' } }, 'deepseek');
      console.log('Call 4: DeepSeek v4-flash OK');
      await cb.onSuccess(CB_DEEPSEEK);
    } catch(e) {
      console.warn('Call 4 DeepSeek v4-flash failed:', e.status || e.message);
...
async function runCronThesisSweep(recentItems, DEEPSEEK_KEY) {
  ...
  await Promise.allSettled(deviceIds.map(async devId => {
    try {
      ...
      const validated = await checkThesisContradictions(openEntries, recentItems, DEEPSEEK_KEY);
...
module.exports = async function handler(req, res) {
  ...
  // Diagnostik DeepSeek API resmi (session 186) — kandidat primary tunggal berbayar,
...
SESI YANG SEDANG/AKAN DIBUKA: ${sesiNow} — mata uang utamanya ${SESI_CURRENCIES[sesiNow]}.`;
    ...
    if (relevantCurrencies.length > 0) {
      ...
      if (!biasRaw && (testDeepseekOnly || testDeepseekProOnly) && DEEPSEEK_KEY && await cb.canCall(CB_DEEPSEEK)) {
        try {
          ...
          console.log('Call 2: DeepSeek v4-flash OK');
          await cb.onSuccess(CB_DEEPSEEK);
      ...
      // diagnostik ke tier produksi. Gemini di bawah TURUN jadi fallback (SambaNova,
  ...
  const _call4Promise = (DEEPSEEK_KEY && deviceId && recentItems.length > 0) ? (async () => {
    try {
...
JANGKAR KEJADIAN SAJA TIDAK CUKUP — kejadiannya juga harus PUNYA BOBOT. Rilis data tier rendah (kredit sektor swasta, inventories, laba korporat kuartalan, izin bangunan, dan sejenisnya) BUKAN tema, walau angkanya meleset dari forecast: dampaknya ke mata uang terlalu kecil untuk jadi paragraf sendiri. Yang layak jadi tema: keputusan/pidato bank sentral, inflasi, tenaga kerja, PDB, PMI utama, peristiwa geopolitik/pasokan, tarif & perang dagang, atau rilis apa pun yang melesetnya besar DAN mekanisme ke mata uangnya bisa kamu sebut konkret. Kalau setelah seleksi ini cuma tersisa satu atau dua tema, itu hasil yang BENAR — jangan ditambal dengan tema ketiga yang lemah.
...
3. SEBELUM memisahkan tema, cek dulu apakah keduanya sebenarnya berkaitan — kalau ya, WAJIB dirajut, bukan ditulis berurutan sebagai laporan terpisah. Kaitan yang paling sering terlewat: data China ke AUD/NZD (China pasar ekspor utamanya), minyak/Hormuz ke CAD dan ke inflasi global, real yield/rate path AS ke semua pair ber-leg USD. Pelanggaran nyata 2026-08-31: paragraf PMI China ditulis persis di atas paragraf AUD tanpa satu pun kalimat yang menghubungkannya, padahal China adalah driver AUD paling langsung hari itu.
   Kalau tema-tema benar-benar tidak berkaitan (misal CB Asia vs data AS, tidak ada irisan driver) — boleh dipisah, tapi maksimal 2 tema independen per output. Tema ketiga ke bawah yang tidak terkait → skip, sebut hanya jika punya magnitude kuat. Tema dengan kaitan kausal lemah/tidak langsung (mis. ekuitas regional sebagai proksi sentimen currency) — SKIP, kecuali magnitude-nya jelas kuat dan disebut dengan mekanisme konkret. Jangan masukkan tema hanya untuk menambah panjang.
...
${xauHistoryBlock}`;
    ...
    // DeepSeek v4-flash (API resmi) — diagnostik terisolasi session 186, lihat
    // DEEPSEEK_URL untuk konteks biaya. max_tokens 1300 SAMA seperti SambaNova V3.2
    ...
    if (testDeepseekOnly) {
      ...
      if (DEEPSEEK_KEY && await cb.canCall(CB_DEEPSEEK)) {
        const t0ds = Date.now();
        try {
          console.log('Call 1: trying DeepSeek v4-flash (API resmi) — diagnostik test_deepseek=1');
          const raw = await aiCall(DEEPSEEK_URL, DEEPSEEK_KEY, DEEPSEEK_MODEL, call1Messages, 1300, 0.25, deepseekTimeout1, {}, { thinking: { type: 'disabled' } }, 'deepseek');
          const elapsed = Date.now() - t0ds;
          if (raw.trim()) {
          ...
          console.warn('Call 1 DeepSeek v4-flash failed:', e.status || e.message);
          await cb.onFailure(CB_DEEPSEEK, AI_CB_THRESHOLD);
        }
    ...
    if (isIsolatedTest) {
      ...
      providerLog.push('deepseek:circuit_open');
      console.log('Call 1: DeepSeek circuit OPEN — skipping to Gemini fallback');
    } else if (!article) {
      providerLog.push('deepseek:no_key');
    }
    ...
    // SambaNova DeepSeek-V3.2 (akun 2, Call 1 prose) — fallback lama di sini, diputus
    // kontrak total 2026-08-12 (billing lapse tak terpulihkan, lihat daun_merah_vendor.md).
    // DeepSeek primary di atas jatuh langsung ke Gemini di bawah kalau gagal.
  ...
  // ── 7. Call 3: Structured Trade Thesis — DeepSeek (primary/satu-satunya) ─────────
  let thesis = null;
  ...
  if (elapsedBeforeCall3 > CALL3_BUDGET_MS) {
    ...
    if (testGeminiOnly) {
      ...
      // Primary/satu-satunya (Plan O-3, 2026-07-18): DeepSeek v4-flash API resmi —
      // promosi dari diagnostik. maxTokens 1200 (naik dari 800, lihat Plan O-1 di atas)
      // mencegah truncation JSON thesis. SambaNova (fallback lama di sini) diputus
      // kontrak total 2026-08-12, lihat daun_merah_vendor.md.
      if (DEEPSEEK_KEY)  call3Providers.push({ url: DEEPSEEK_URL, key: DEEPSEEK_KEY, model: DEEPSEEK_MODEL, label: 'DeepSeek v4-flash', timeout: 15000, provider: 'deepseek', circuit: CB_DEEPSEEK, maxTokens: 1200, extraBody: { response_format: { type: 'json_object' }, thinking: { type: 'disabled' } } });
    }
  ...
  if (isCronCall && DEEPSEEK_KEY && recentItems.length > 0) {
    runCronThesisSweep(recentItems, DEEPSEEK_KEY).catch(e => console.warn('Cron thesis sweep failed:', e.message));
  }
