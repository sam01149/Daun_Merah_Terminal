...
const POLICY_EPOCHS = [
  ...
  { v: 39, from: '2026-09-02T01:22:05Z', kind: 'fix',    impact: 'entry',    label: 'Fix celah gate arah v38: kode currency di dalam sebutan pair generik ("pair AUD/NZD") tidak lagi bisa jadi target kata arah terdekat, dan klausa "tidak ada data yang mendukung X" tidak lagi salah dibaca sebagai "X terjadi" — sebelumnya "...menekan pair AUD/NZD..." + "...tidak ada data NZD yang mendukung penguatan..." sama-sama menghasilkan sinyal "mendukung" palsu yang menggagalkan syarat nol-pendukung, meloloskan inversi arah nyata (AUD/NZD:1788308131973, policy_v 38, commit 355cc7a)' },
  { v: 40, from: '2026-09-02T01:44:49Z', kind: 'mixed',  impact: 'context',  label: 'Kejujuran & konsistensi checklist/prompting (audit 2026-09-02): (1) prompt Call 1 sekarang eksplisit menjelaskan konvensi base/quote (bullish/bearish -> currency mana yang menguat) -- sebelumnya sama sekali tidak dijelaskan, kemungkinan besar akar bug arah_driver_berlawanan yang berulang 3x; (2) field konflik di fundamental_bias sekarang didefinisikan (bukan tempat parkir bukti pendukung sekunder); (3) verdict TIDAK LAGI laporan bebas AI -- diturunkan KODE dari checklist_pct+conflict+gate_risk_management (_deriveAatasVerdict), menutup mismatch nyata (checklist_pct 78 tapi verdict "PERTIMBANGKAN", harusnya "SIAP TRADE"); CARA checklist_pct dihitung TIDAK disentuh, tetap opini AI; (4) final_validation (Step 8 COT/retail) dipaksa kode ke "tidak tersedia" -- Call 2 tidak pernah menerima data COT/retail sama sekali, jadi nilai apa pun yang pernah ditulis di sana adalah karangan; (5) sentimen options CME sekarang benar-benar dikirim ke Call 2 cabang XAU (rrBlock) -- checklist sudah lama menyebutnya sebagai konfirmasi Step 4 tapi datanya cuma sampai ke Call 1. AATAS_PROMPT_VERSION dinaikkan ke 3.' },
  { v: 41, from: '2026-09-02T01:57:08Z', kind: 'policy', impact: 'entry',    label: 'Gate keras baru: technical.struktur_vs_bias==="berlawanan" (enum 4-state: selaras/netral/campur/berlawanan) memaksa NO TRADE -- sebelumnya cuma imbauan teks Step 4, kasus nyata AUDNZD=X:1788182128048 mengakui struktur H4 berlawanan bias tapi tetap keluar setup 78%. "campur"/"netral" SENGAJA tidak digate (konservatif, hindari salah tembak struktur ambigu). AATAS_PROMPT_VERSION dinaikkan ke 4.' },
  { v: 42, from: '2026-09-04T02:29:51Z', kind: 'fix',    impact: 'entry',    label: 'Gate RR Step 6 ditegakkan KODE (_enforceAatasRrGate), bukan lagi klaim bebas model. Audit 16 setup populasi AATAS (2026-09-04): 12 menulis angka RR di gate_risk_management.note yang TIDAK cocok dengan level entry/sl/tp di respons yang SAMA (9 di antaranya level-nya tidak pernah di-refine), dan klaimnya hampir selalu mendarat di 2,1-2,6 -- persis di atas ambang -- sementara RR aktual berkisar 1,02-11,90; pola menulis-angka-yang-lolos, bukan menghitung. Dua setup ber-RR aktual DI BAWAH 1:2 tetap pass=true lalu jadi setup hidup (AUDNZD=X:1788480927701 rr 1,24; AUDNZD=X:1788221723137 rr 1,02) padahal Step 6 sendiri memerintahkan NO TRADE. Sekarang structured.risk_reward (dihitung kode dari level final) yang memutuskan; klaim model tetap disimpan di dalam note sebagai jejak audit. Ambang 1:2 BUKAN angka baru -- sudah tertulis di prompt Step 6 sejak awal, yang belum ada cuma penegakannya. Pola sama v40 poin (3)/(4). Laju entry bisa turun sedikit (2 dari 16 setup historis akan tertolak) -- itu memang tujuannya. Prompt TIDAK disentuh, AATAS_PROMPT_VERSION tetap 4.' },
  { v: 43, from: '2026-09-04T03:01:41Z', kind: 'policy', impact: 'entry',    label: 'Skor fundamental jadi DUA SISI yang wajib berjumlah 100 (case_bullish_pct + case_bearish_pct), menggantikan score_pct laporan bebas AI. AATAS_PROMPT_VERSION dinaikkan ke 5. Dipicu user (2026-09-04): "kalau bullish 70%, gamungkin bearish juga 70%". Terbukti di 16 setup populasi AATAS -- AUD/NZD dapat 55/72/72/68 saat bullish DAN 70/68/62/62 saat bearish (rata-rata 66,8 vs 65,5, tidak bisa dibedakan), EUR/USD dapat angka SAMA PERSIS (62) untuk dua arah berlawanan, dan sebaran seluruh populasi cuma 55-75 -- sistem TIDAK PERNAH sekali pun mengatakan fundamentalnya lemah. Akar masalah di prompt bukan di model: spesifikasi JSON cuma menulis "score_pct":0-100 tanpa satu kalimat pun tentang angka itu mengukur apa, dan _evaluateAatasGate1 tidak pernah membacanya -- tak terdefinisi + tak dipakai = model mengisinya sebagai keyakinan atas arah yang SUDAH dipilih, yang secara desain tidak pernah bisa rendah. Sekarang: score_pct diturunkan KODE dari sisi yang menang (bukan lagi diminta dari model), margin_pct = selisih dua sisi jadi ukuran keyakinan yang sesungguhnya (52 vs 48 akhirnya bisa dibedakan dari 80 vs 20). Gate 1 menegakkan tiga hal baru: skor_dua_sisi_hilang, skor_dua_sisi_tidak_100, arah_vs_skor_berlawanan (arah yang dideklarasikan wajib sama dengan sisi yang menang) -- diukur lewat auto_guard_stats:gate1_skor_dua_sisi. SENGAJA TIDAK ADA ambang atas margin_pct: kalibrasinya belum terukur, memasang ambang atas angka yang belum tervalidasi adalah kesalahan yang sudah berulang di proyek ini. Informasional dulu. RISIKO: gate skema ini fail-closed, jadi kalau model rutin mengabaikan field baru laju entry bisa jatuh ke nol -- pantau counter di atas beberapa siklus pertama (preseden Gate E hari pertama nol entry).' },
  // v44 (2026-09-06, PLAN AC Tahap 1): 'policy' impact 'context' — ISI informasi yang
  // dilihat Call 1 berubah (data rilis domestik yang selama ini absen), tidak ada
