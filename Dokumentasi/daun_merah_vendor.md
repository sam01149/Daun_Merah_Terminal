...
| Mistral AI | `MISTRAL_API_KEY` | Free tier — satu-satunya provider Translate NEWS ke Bahasa Indonesia (S272, detail `daun_merah_ai.md` §item 10). **BERMASALAH sejak 2026-09-06 (ditemukan S351):** endpoint `/v1/chat/completions` selalu balas `429` dengan header `x-ratelimit-limit-req-minute: 0` — LIMIT-nya sendiri di-set 0 oleh Mistral, di level akun/API key, berlaku untuk SEMUA prompt lewat endpoint ini apa pun isinya (bukan spesifik ke "penerjemah" — endpoint chat completions Mistral cuma satu, tidak ada billing terpisah per jenis tugas). `/v1/models` (endpoint gratis, tidak lewat billing) tetap 200 OK, key masih valid/tidak revoked. **Root cause TERKONFIRMASI dari dokumentasi resmi Mistral (help.mistral.ai/en/articles/698531):** ini BUKAN soal saldo/trial habis — user sudah top-up $10 (sisa $9,96) & set key "never expires", TETAP 429/limit 0 saat dites ulang live. Mistral: **"Prepaying credits adds funds to your account; it does not change your tier or rate limits."** Tier (penentu req/menit) naik dari AKUMULASI TAGIHAN TERPAKAI (bukan saldo tersedia) setelah ambang $20/$100/$500/$2000, dan butuh mode **"Pay-as-you-go" aktif** (setting terpisah dari sekadar isi saldo) — dicek/diaktifkan di https://admin.mistral.ai/plateforme/limits. Saldo $9,96 nyaris utuh justru menunjukkan akun belum pernah benar-benar tertagih pemakaian, makanya masih tier terendah. **Catatan penting biar tidak salah paham lagi:** angka "jatah 1000/hari" (`mistral_newstranslate`, pemakaian 6/1000 saat ditemukan) itu PENJAGA INTERNAL KITA SENDIRI di `_ai_guard.js` (counter Redis kita) — sama sekali BUKAN kuota resmi dari Mistral, tidak terhubung ke rate-limit akun mereka. Circuit breaker `ai:mistral:newstranslate` OPEN otomatis, translate NEWS berhenti total sampai limit pulih (self-healing, tidak perlu deploy ulang). **Syarat lanjut:** user cek/aktifkan Pay-as-you-go di admin.mistral.ai/plateforme/limits, atau hubungi support Mistral kalau tetap 0 — lihat `daun_merah_progress.md`. |
