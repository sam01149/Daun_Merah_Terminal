...
| `setup-tp-sl-watch.yml` | Ditulis `*/5`, **realisasi ~3-4x/hari** | Fallback evaluasi TP/SL setup auto-entry (Q-7) — jalur utamanya daemon Railway; workflow ini cadangan kalau daemon mati. **Cadangan yang lemah:** dengan realisasi ~3-4x/hari, kalau Railway mati, deteksi TP/SL bisa telat berjam-jam. Belum diperbaiki — butuh keputusan biaya user (cron-job.org gratis atau Railway Hobby $5/bln), lihat `daun_merah_progress.md` S336 |
| `warm-and-watch.yml` | Ditulis `*/10`, **realisasi ~3-4x/hari** | GABUNGAN 3 workflow lama (2026-08-29): health probe VPS (`admin?action=health`, header `x-admin-secret` — BUKAN `x-cron-secret`), refresh retail sentiment, warm RSS + translate backfill, dan warm makro (`risk-regime`/`real-yields`/`cot`) yang dijalankan SETELAH health probe supaya clear-on-recovery langsung diikuti pengisian ulang. Tiap step `continue-on-error`; workflow baru gagal kalau SEMUA step gagal |
| `setup-log-archive.yml` | Harian 04:00 UTC | Arsip `setup_log_auto:v1` sebelum cap 200 + (sejak 2026-08-29) cek staleness `INFLATION_EXPECTATIONS` dengan penjaga hari Senin — dulu workflow terpisah `inflation-staleness-check.yml` |
...
**Realisasi jadwal jauh di bawah yang ditulis (diukur 2026-08-29, S336).** `gh run list` lintas 3 hari: SEMUA workflow terjadwal dilayani **~3-4 run/hari**, terlepas dari cron yang ditulis — `*/5` (harusnya 288/hari) dan `*/15` (96/hari) sama-sama ~3-4. Total seluruh akun ~24 run terjadwal/hari, terbagi round-robin, dan repo ini PUBLIC jadi bukan soal kuota menit privat. **Konsekuensi praktis: jangan pernah merancang TTL cache, deteksi outage, atau alarm dengan asumsi jadwal ditepati** — dua bug senyap sudah lahir dari asumsi itu (TTL `latest_article` S334, deteksi "sumber pulih" di `?action=health` S336). Jumlah workflow terjadwal sudah dipangkas 8 -> 6 untuk membebaskan slot; hipotesis "jatah per-akun" belum terbukti (bisa saja per-workflow), lihat `daun_merah_progress.md` S336.

Semua workflow di atas autentikasi ke `api/*.js` lewat header `x-cron-secret` — **KECUALI `admin?action=health` yang memakai `x-admin-secret`** (dua-duanya dicocokkan ke `CRON_SECRET` yang sama, jadi gampang tertukar; ketahuan lewat 401 saat penggabungan workflow 2026-08-29), dicocokkan ke `CRON_SECRET` di kode (`api/_app_key.js`, `api/_ratelimit.js` — whitelist otomatis, tidak kena rate limit per-IP).
...
|---|---|---|
| Google AI Studio (Gemini) | `GEMINI_API_KEY` | Free tier tersedia untuk model tertentu, dengan input/output token gratis; layak dipakai lebih dulu untuk uji diagnostik auto-entry virtual. Kuota bergantung pada model dan project; pemakaian proyek yang pernah terukur hanya sekitar 20 request/hari untuk alias flash lama (guard 16/hari), bukan angka 1.500 RPD yang sudah basi dan bukan batas universal. Primary/satu-satunya provider Analisis Fundamental & AI Coach Jurnal sejak 2026-08-12, DITAMBAH Translate NEWS (S351-352, 2026-09-06, model `gemini-flash-lite-latest` — beda model dari Fundamental/Jurnal jadi pool kuota terpisah, lihat baris Mistral di bawah untuk alasan pindah + `daun_merah_ai.md` §item 10/§4). **Status kandidat paid 2026-09-16:** Gemini Plus user tidak memberi kredit/kuota API. Paid tier baru perlu dipertimbangkan bila uji free tier gagal kapasitas/stabilitas; Cloud Billing + prepay minimal US$5/ekuivalen lokal. Profil Google Payments personal yang sama dapat dicoba, dan Google Cloud Indonesia menerima Mastercard debit/kredit; ini jalur pembayaran berbeda dari DeepSeek, bukan jaminan approval bank. Penggantian penuh DeepSeek belum dilakukan: auto-entry virtual masih perlu mode diagnostik, structured JSON + validator, serta pemisahan budget/circuit public vs eksperimental. Detail: `daun_merah_riset.md` 2026-09-16. |
| DeepSeek (API resmi) | `DEEPSEEK_API_KEY` | Berbayar dari saldo top-up user (bukan free tier); model `deepseek-v4-flash` (auto-upgrade ke build V4-Flash-0731 sejak 2026-07-31, server-side, harga tak berubah) untuk hampir semua fitur, KECUALI Call 1 Ringkasan Berita yang dipromosikan ke `deepseek-v4-pro` 2026-08-17 (detail chain per-fitur & alasan promosi: `daun_merah_ai.md`); primary/satu-satunya provider hampir semua fitur AI sejak SambaNova diputus (lihat di bawah). **Status top-up 2026-09-16:** akun ini sebelumnya sukses melalui PayPal (18 Jul) dan Card (7 Agu), maka Taplus Muda bukan terbukti tidak kompatibel. Setelah kegagalan pertama 7 Agu, semua percobaan Card/Google Pay/PayPal dibatalkan, termasuk rentetan 15-16 Sep; pola lintas-metode ini lebih konsisten dengan hold risiko pada gateway/akun daripada satu kartu BNI. Hentikan retry; kirim daftar invoice gagal + dua invoice sukses ke `api-service@deepseek.com` agar status pembayaran akun diperiksa. |
| DeepSeek — harga resmi (dicek ulang 2026-09-16, per 1 juta token, USD) | — | **v4-flash**: input cache-hit $0,0028, cache-miss $0,14, output $0,28. **v4-pro**: input cache-hit $0,003625, cache-miss $0,435, output $0,87. Tarif peak/off-peak lama di baris ini sudah tidak berlaku menurut halaman pricing resmi terkini; pakai angka ini untuk proyeksi baru. [Pricing resmi](https://api-docs.deepseek.com/quick_start/pricing) |
| Qwen 3.8 API — Alibaba Cloud Model Studio (dicek 2026-09-16; **belum diaktifkan, Qwen user tetap khusus kantor**) | — | **`qwen3.8-flash`, Global:** CNY 0,8 input / 2,7 output per 1 juta token (≈ $0,118 / $0,399 dengan kurs referensi CNY 6,7628 per USD). **`qwen3.8-max`, Global:** CNY 12 / 36 (≈ $1,774 / $5,323). Jalur Singapore International lebih mahal: Flash CNY 1,094 / 3,427; Max CNY 14,988 / 44,965. Free quota Qwen 3.8 hanya China (Beijing), tidak tersedia di region lain. Jadi Flash hampir setara DeepSeek Flash: input Global 15% lebih murah, tetapi output 43% lebih mahal; Max sekitar 4,1x input dan 6,1x output DeepSeek Pro. Harga/region perlu diverifikasi pada project sebelum transaksi. [Pricing resmi Model Studio](https://help.aliyun.com/en/model-studio/model-pricing), [kurs referensi 2026-09-16](https://www.safe.gov.cn/AppStructured/hlw/RMBQuery.do?COLLCC=546877760) |
| ModelHub.my — gateway multi-model (dicek 2026-09-16; **BELUM DITERIMA untuk Daun Merah**) | `MODEL_HUB` tersimpan lokal, tetapi **belum dipakai kode** | Gateway OpenAI-compatible ke model pihak ketiga, mencantumkan DeepSeek V4 Flash/Pro dan Qwen3.8 Max. Audit kode 2026-09-16: tidak ada referensi `MODEL_HUB`, endpoint, atau parameter `model`; key lokal bukan bukti pemakaian dan tidak ada request Daun Merah yang dapat muncul di dashboard ModelHub sampai integrasi dibuat. Harga yang ditampilkan gateway: DeepSeek Flash $0,0135/$0,027 dan Qwen3.8 Max $0,4425/$1,3275 per 1 juta token input/output — cek dashboard sebelum top-up karena harga/availability dapat berubah tanpa pemberitahuan. **Jika key dipakai oleh aplikasi lain, jejak penggunaan pasti ada:** terms mencatat timestamp, model, token masuk/keluar, latency, status, IP, dan ID API key; homepage juga menjanjikan dashboard log per request. Prompt/respons diteruskan ke provider pihak ketiga dan dapat diproses/disimpan sementara untuk billing, keamanan, diagnostik, serta anti-abuse. Karena gateway tidak menjelaskan di halaman publik backend/provider spesifik untuk setiap rute maupun retention period, jangan kirim data kantor/pribadi sensitif dan jangan pakai key/account kantor untuk proyek pribadi. Untuk auto-entry, perlu konfirmasi tertulis jalur provider + ToS yang berlaku dan uji terisolasi lebih dahulu. [Terms & Privacy](https://www.modelhub.my/terms), [catalog & harga](https://www.modelhub.my/) |
| Kandidat pengganti DeepSeek — seleksi 2026-09-16 | — | **Gemini:** satu-satunya kandidat siap pakai karena sudah aktif, tetapi kuota akun terukur ~20 request/hari sehingga hanya layak fallback fitur manual, bukan pengganti penuh/auto-entry tanpa billing & uji. **Mistral:** DITOLAK (Usage Policy 11 Jun 2026 melarang investment advice/financial guidance). **Together AI:** DITOLAK (Terms melarang financial information of any nature). **OpenAI API:** tidak cocok jalur auto-entry karena policy melarang automated high-stakes financial decisions tanpa human review; kandidat manual saja bila user kelak memilih akun berbayar baru. Detail sumber & alasan: `daun_merah_riset.md` 2026-09-16. |
| Mistral AI | `MISTRAL_API_KEY` | Free tier — **SUDAH TIDAK DIPAKAI kode produksi mana pun sejak 2026-09-06 (S351-352)**, digantikan Gemini flash-lite untuk Translate NEWS (lihat baris di atas). Ditinggalkan karena endpoint `/v1/chat/completions` selalu balas `429` dengan header `x-ratelimit-limit-req-minute: 0` — LIMIT-nya sendiri di-set 0 oleh Mistral di level akun/API key, berlaku untuk SEMUA prompt (bukan spesifik "penerjemah", dan ganti API key TIDAK membantu — dites live, limit tetap 0 dengan key baru). **Root cause TERKONFIRMASI dari dokumentasi resmi Mistral (help.mistral.ai/en/articles/698531):** BUKAN soal saldo/trial habis (user top-up $10, sisa $9,96, key "never expires") — prepaid credit TIDAK menaikkan tier/rate-limit, perlu AKUMULASI TAGIHAN TERPAKAI (ambang $20/$100/$500/$2000) + mode **"Pay-as-you-go" aktif** (dicek/diaktifkan di admin.mistral.ai/plateforme/limits). User menolak tunggu reset 25 hari atau bayar, jadi provider dipindah alih-alih ditunggu. **Kalau nanti user aktifkan Pay-as-you-go dan mau pakai Mistral lagi** (mis. sebagai fallback tambahan): kode `api/_news_translate.js` sudah tidak memanggilnya sama sekali, perlu diwire ulang dari nol, bukan tinggal reaktivasi flag. |
...
**OpenCode Zen (model "Big Pickle", gratis, key `OPENCODE_ZEN`) — DITOLAK 2026-09-06 (S352), evaluasi untuk Translate NEWS.** Lolos gate ToS (tidak ada larangan "not in production" atau larangan data finansial/keputusan investasi seperti kasus NVIDIA/z.ai — cuma klausul wajar "boleh pakai data akun gratis untuk improve model", risiko rendah karena headline berita memang sudah publik). GAGAL di sisi TEKNIS: tes live 5x panggilan menunjukkan latensi 6-27 detik SANGAT TIDAK KONSISTEN dan TIDAK PROPORSIONAL jumlah headline (batch 2 headline = 6 detik, batch 5 headline = 14 detik, satu kalimat pendek tunggal = 27 detik) — root cause: model ini SELALU mengaktifkan mode "thinking" (`reasoning_content` di respons, sampai 831 token cuma untuk menerjemahkan 1 kalimat pendek) dan TIDAK BISA dimatikan lewat parameter API manapun yang dicoba (`reasoning_effort=minimal/low/none`, `enable_thinking=false`, `reasoning.effort`/`reasoning.enabled` nested) — beda dari Gemini 3.x yang punya mitigasi `reasoning_effort:'low'` yang benar-benar bekerja. Ditambah kena error `FreeUsageLimitError` cuma dari ~8 kali panggilan tes dalam beberapa menit — jatah gratisnya kecil. Kombinasi latensi tak terduga (bisa >20 detik) + limit function Vercel 20 detik = risiko men-timeout SELURUH `api/feeds.js` (bukan cuma translate yang gagal, tapi fitur NEWS utama ikut down) — jangan diusulkan ulang untuk fitur otomatis apa pun kecuali OpenCode Zen mengubah perilaku model ini.
...
| Vendor | Fungsi | Tier |
|---|---|---|
| **Lightweight Charts v4** (`unpkg.com/lightweight-charts@4`, fallback `cdn.jsdelivr.net`) | Library chart candlestick di frontend — dipakai di **tab Chart Posisi `dev-auto-entry.html`**. Apache 2.0, dibuat TradingView sendiri, tanpa watermark, tanpa delay data. Data aktual dari Deriv WebSocket (real-time per detik). **Koreksi S341 (2026-08-31):** entri lama di sini menyebut library ini "menggantikan TradingView embed di tab Teknikal sejak Session 215" — itu tidak sesuai kode. Tab TEK `index.html` masih memakai widget embed TradingView (`createTVChart()`, baris ~16880) sampai sekarang | Free, Apache 2.0, tanpa API key |
| **TradingView Advanced Chart Widget** (`s3.tradingview.com/tv.js`) | Chart candlestick tab TEK di `index.html` (`createTVChart()`) — embed iframe cross-origin, simbol per pair di `TEK_TV_SYM`, studi MA 50 / MA 200 / RSI 14 dipasang lewat opsi `studies`. **Batas yang perlu diingat:** widget ini stateless — gambar manual user (trendline/garis horizontal/fibo) hidup di dalam iframe milik TradingView, tidak ada API save/load, tidak ada login akun, dan same-origin policy melarang kita membacanya. Jadi gambar TIDAK bisa dibuat bertahan setelah refresh selama masih memakai widget ini (persist antar-tab sudah diakali dengan memakai ulang iframe, S341) | Free, tanpa API key |
| **TradingView Advanced Charts / Charting Library** (versi berlisensi, `save_load_adapter` + `auto_save`) | **BUKAN OPSI — blocker lisensi.** Ini satu-satunya produk resmi TradingView yang bisa menyimpan gambar chart. Dicek 2026-08-31 ke halaman resmi: *"At this time, we don't provide the Advanced Charts and the Trading Platform libraries for personal use, hobbies, studies, or testing"* — lisensi hanya diberikan ke **perusahaan** untuk proyek web publik/aplikasi. Daun Merah proyek pribadi, jadi tidak memenuhi syarat. Halaman yang sama mengarahkan pemakaian pribadi ke Lightweight Charts. Pola blocker sama dengan ToS z.ai: jangan diusulkan ulang tanpa perubahan lisensi dari pihak TradingView. Sumber: https://www.tradingview.com/free-charting-libraries/ | Tidak tersedia untuk kita |
