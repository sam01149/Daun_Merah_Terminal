...
| Mistral AI | `MISTRAL_API_KEY` | Free tier — satu-satunya provider Translate NEWS ke Bahasa Indonesia (S272, detail `daun_merah_ai.md` §item 10). **BERMASALAH sejak 2026-09-06 (ditemukan S351):** endpoint `/v1/chat/completions` selalu balas `429` dengan header `x-ratelimit-limit-req-minute: 0` — LIMIT-nya sendiri di-set 0 oleh Mistral (bukan sekadar kepakai habis; pemakaian harian kita cuma 6/1000 saat ditemukan), sementara `/v1/models` tetap 200 OK (key masih valid, bukan revoked). Pola khas trial habis/belum pasang metode pembayaran/akun ditahan. Circuit breaker `ai:mistral:newstranslate` OPEN otomatis, translate NEWS berhenti total sampai limit pulih (self-healing, tidak perlu deploy ulang). **Syarat lanjut:** user cek console.mistral.ai (billing/plan) — lihat `daun_merah_progress.md`. |
