...
### Update 2026-07-28 — Outage TLS cert expired (vendor-side, bukan bug kode)

Fitur Risk Reversal 25D (tab Analisa, semua pair termasuk XAU/USD) sempat kosong total — endpoint `action=risk-reversal` balikin `available:false`. Root cause ditemukan via diagnostic sementara di `correlations.js` (tangkap `e.cause` dari fetch yang gagal): **sertifikat TLS `api.scraperapi.com` sudah expired** (`notAfter=Jul 28 00:42:41 2026 GMT`, dikonfirmasi `openssl s_client`), bukan CME ubah format response (dicek langsung raw response CME via proxy baca lain — format masih identik/valid). Murni outage vendor, di luar kendali kode kita — tidak ada mitigasi selain tunggu ScraperAPI perbarui sertifikatnya sendiri. Field `debug` di response `action=risk-reversal` DIPERTAHANKAN permanen (bukan cuma sementara) supaya insiden serupa ke depan bisa didiagnosis dari response API langsung, tanpa perlu akses log Vercel.

**Konfirmasi resmi (status.scraperapi.com, dicek user):** insiden "Akamai protected domains Success Rates" berstatus **Investigating** sejak **2026-07-27 02:04 MDT**, deskripsi: *"Due to Akamai releasing a change in their security mechanism this weekend, we are currently experiencing a lower than normal overall success rate for Akamai protected domains."* CME (`cmegroup.com`) memang salah satu domain berproteksi Akamai (alasan awal ScraperAPI dipilih — lihat catatan "kompetitor" di atas), jadi ini match dengan root cause: TLS cert expired kemungkinan efek samping ScraperAPI buru-buru ubah infrastruktur untuk merespons perubahan Akamai tsb, bukan kejadian terpisah. Overall API/Website/Billing ScraperAPI berstatus Operational — cuma jalur khusus domain Akamai-protected yang kena. Tidak ada ETA resolusi dari vendor per saat ini.

**Mitigasi ditemukan (2026-07-28, sama sesi):** user tanya ulang "benar-benar tidak ada yang bisa dilakukan?" — ternyata ada: **r.jina.ai Reader API** (gratis, tanpa signup/key, dari Jina AI) fetch dari IP mereka sendiri yang TIDAK kena block WAF Akamai/CME (beda dari IP datacenter Vercel). Dites live saat ScraperAPI masih down: parse 6/6 pair CME CVOL (termasuk XAU/USD) berhasil sempurna. `correlations.js` (commit `0f738a0`) sekarang coba ScraperAPI/direct dulu, fallback otomatis ke Jina Reader kalau gagal — **bukan ganti vendor permanen**, cuma jaring pengaman tambahan yang otomatis idle lagi begitu ScraperAPI pulih. Risiko dipakai jangka panjang: Jina Reader didesain untuk baca halaman ke LLM (bukan API proxy resmi), berpotensi rate-limit kalau volume naik drastis — untuk kasus ini aman karena RR cuma di-refresh 1x/jam (cache `RR_CACHE_TTL` 3600).
