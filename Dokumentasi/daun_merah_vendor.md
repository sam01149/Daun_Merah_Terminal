...
| Vendor | Fungsi | Tier | Env var |
|---|---|---|---|
| **FRED (Federal Reserve Economic Data, St. Louis Fed)** | Sumber data utama seri makro AS (yield, inflasi, GDPNow, labour market rule-based, dll) — dipakai di `real-yields.js`, `risk-regime.js`, `_labour_market.js`, `admin.js`. **Dua jebakan yang sudah menggigit (audit 2026-08-29):** (1) endpoint keyless `fredgraph.csv` **mengabaikan** `sort_order`/`limit` (selalu kirim seri PENUH urut ASCENDING) dan headernya sudah berubah dari `DATE,` jadi `observation_date,` — dua fungsi memakai asumsi lama dan rusak, lihat `daun_merah_progress.md` S336; (2) yield 10Y non-USD memakai seri OECD BULANAN `IRLTLT01*M156N` yang observasi terakhirnya basi 3-8 bulan | Free (API key wajib untuk endpoint `api.stlouisfed.org`; `fredgraph.csv` tanpa key) | `FRED_API_KEY` |
| **ECB Data API** (`data-api.ecb.europa.eu`) | Yield Eropa + suku bunga acuan ECB | Free, tanpa API key | — |
| **Bank of England, Bank of Japan, Bank of Canada, RBA, RBNZ, SNB** (situs resmi masing-masing) | Suku bunga acuan live per bank sentral non-Fed/ECB (`_cb_rates.js`) | Free, scraping halaman publik. **RUSAK per audit 2026-08-29 — hanya BoC (Valet API) yang masih jalan.** BoE/BoJ/RBNZ/SNB: halaman balas 200 tapi regex `_cb_rates.js` 0 match (struktur halaman berubah). RBA: HTTP 403 (memblokir bot). Ditambah `scrapeUSD` (FRED CSV) juga rusak, `cb_rates_live_v2` di Redis produksi cuma berisi EUR+CAD — 6 currency lain diam-diam pakai tabel statis `CB_FALLBACK`. Bukti + rencana perbaikan (BoE IADB sudah diverifikasi jadi pengganti) di `daun_merah_progress.md` S336 | — |
