...
| Mistral AI | `MISTRAL_API_KEY` | Free tier — satu-satunya provider Translate NEWS ke Bahasa Indonesia (S272, detail `daun_merah_ai.md` §item 10). **BERMASALAH sejak 2026-09-06 (ditemukan S351):** endpoint `/v1/chat/completions` selalu balas `429` dengan header `x-ratelimit-limit-req-minute: 0` — LIMIT-nya sendiri di-set 0 oleh Mistral, di level akun/API key, berlaku untuk SEMUA prompt lewat endpoint ini apa pun isinya (bukan spesifik ke "penerjemah" — endpoint chat completions Mistral cuma satu, tidak ada billing terpisah per jenis tugas). `/v1/models` (endpoint gratis, tidak lewat billing) tetap 200 OK, key masih valid/tidak revoked — pola khas trial habis/belum pasang metode pembayaran/akun ditahan. **Catatan penting biar tidak salah paham lagi:** angka "jatah 1000/hari" (`mistral_newstranslate`, pemakaian 6/1000 saat ditemukan) itu PENJAGA INTERNAL KITA SENDIRI di `_ai_guard.js` (counter Redis kita, dibuat supaya bug kode tidak memanggil Mistral tanpa batas) — sama sekali BUKAN kuota resmi dari Mistral dan tidak terhubung ke rate-limit akun mereka. Jangan dianggap sebagai bukti "belum kepakai habis di sisi Mistral"; limit yang sebenarnya memblokir adalah header `x-ratelimit-limit-req-minute` di atas. Circuit breaker `ai:mistral:newstranslate` OPEN otomatis, translate NEWS berhenti total sampai limit pulih (self-healing, tidak perlu deploy ulang). **Syarat lanjut:** user cek console.mistral.ai (billing/plan) — lihat `daun_merah_progress.md`. |
