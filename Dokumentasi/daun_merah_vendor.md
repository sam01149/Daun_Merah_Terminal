...
**Fix yang diterapkan** (bukan ganti vendor — perbaiki pola pemakaian di kode sendiri):
- `RR_CACHE_TTL` di `correlations.js` dinaikkan dari 3600 (1 jam) → **21600 (6 jam)** — konsisten dengan TTL rate-path.js (4h) dan fundamental_analysis (6h). Motong konsumsi CVOL dari maks 144/hari jadi maks 24/hari.
- `market-digest.js`: blok SKEW OPSI sekarang dapat penanda umur eksplisit (`[data X jam lalu]`) dan dimasukkan ke cakupan CATATAN STALENESS yang sudah ada — AI diberi tahu eksplisit kalau data ini bisa beberapa jam basi dan diinstruksikan memberi bobot lebih ke headline yang lebih segar kalau ada konflik, menutup celah akurasi dari TTL yang lebih longgar.
- Diverifikasi via simulasi isolated (4 skenario age-tag) + test suite 190/190 tetap hijau.
