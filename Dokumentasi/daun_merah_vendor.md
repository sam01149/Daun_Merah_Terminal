...
| DeepSeek (API resmi) | `DEEPSEEK_API_KEY` | Berbayar dari saldo top-up user (bukan free tier); model `deepseek-v4-flash` (auto-upgrade ke build V4-Flash-0731 sejak 2026-07-31, server-side, harga tak berubah) untuk hampir semua fitur, KECUALI Call 1 Ringkasan Berita yang dipromosikan ke `deepseek-v4-pro` 2026-08-17 (detail chain per-fitur & alasan promosi: `daun_merah_ai.md`); primary/satu-satunya provider hampir semua fitur AI sejak SambaNova diputus (lihat di bawah). **Status top-up 2026-09-16:** gateway dapat menolak kartu Indonesia; Google Pay bukan rail terpisah karena tetap menagihkan kartu yang sama. Jika kartu Debit BNI Mastercard Taplus Muda ditolak, perlu konfirmasi bank untuk transaksi e-commerce lintas negara/merchant tersebut; bila bank tidak menolak, kirim bukti error ke `api-service@deepseek.com` agar gateway/region akun diperiksa. |
