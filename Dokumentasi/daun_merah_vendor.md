...
| **Railway** | VPS daemon `vps/daemon.js` (heartbeat, streaming candle Deriv WS, alert berita/harga real-time, scheduler cadangan market-digest/ohlcv_sync, cron auto-entry) — lihat `vps/README-deploy.md` | **BERMASALAH (2026-08-20, Session 323):** free plan sekarang WAJIB mode "Serverless" (auto-sleep saat idle) — bertentangan dengan kebutuhan daemon yang harus selalu hidup (heartbeat/cron/WebSocket). Deploy gagal sampai serverless flag dinyalakan ATAU pindah plan berbayar ATAU migrasi platform lain (Render sedang dicoba ulang, lihat `vps/README-deploy.md`) | `UPSTASH_REDIS_REST_URL/TOKEN`, `DERIV_APP_ID`, `CRON_SECRET`, `TELEGRAM_BOT_TOKEN/CHAT_ID`, `VAPID_PUBLIC/PRIVATE_KEY` — daftar lengkap di `vps/README-deploy.md` §6 |
