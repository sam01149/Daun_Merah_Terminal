...
> **Last updated:** 2026-07-13 (session 163 — user minta tes Z.ai GLM 4.7 via Cerebras sebagai kandidat baru Call 1. Ditambahkan jalur diagnostik terisolasi `?test_glm=1` (pola sama seperti Hermes/Nemotron — skip semua tier lain, hasil tidak pernah masuk cache produksi), dites live: **DITOLAK** — HTTP 400 "Please reduce the length of the messages or completion. Current length is 13029 while limit is 8192", context window model ini di tier "Preview" Cerebras cuma 8192 token, jauh di bawah prompt Call 1 (~13K token dengan headline+kalender+OHLCV). Bug sampingan ditemukan & difix selagi diagnosis: `aiCall()` di market-digest.js cuma baca error nested ala OpenAI (`error.message`) padahal Cerebras balas error flat (`{message,type,param,code}`) — root cause kenapa error 400 sebelumnya cuma kelihatan "HTTP 400" tanpa detail di log manapun; sekarang baca dua-duanya. `?test_glm=1` TETAP ada di kode (tidak dihapus, jaga-jaga kalau Cerebras menaikkan context cap Preview-nya nanti) tapi tidak direkomendasikan jadi kandidat aktif. Sekaligus terkonfirmasi: akun SambaNova (`SAMBANOVA_API_KEY_CALL1`) ternyata paid Developer Tier, bukan free tier — kredit signup $5 habis, minta bayar. gpt-oss-120b (OpenRouter) dipromosikan jadi **primary Call 1 untuk live/on-demand request** (SambaNova digeser jadi fallback 1, otomatis kepakai lagi kalau limitnya reset/di-top-up). Ide lanjutan user: Nemotron Ultra (Ollama, kualitas lebih bagus tapi latency liar 7-41s) diaktifkan lagi **KHUSUS untuk 3 jadwal cron session-open** (GitHub Actions, bukan live request) — hasil cron ditulis ke cache buat semua user jadi latency liarnya tidak pernah dirasakan user yang menunggu sinkron. Ditambah guard baru `CALL1_HARD_BUDGET_MS` (48s dari `handlerStart`) supaya cascade Nemotron(45s)→gpt-oss(15s)→SambaNova(22s)→Groq(15s) kalau gagal berantai tidak mendorong Call 1 sendirian tembus `maxDuration` 60s Vercel dan membunuh seluruh function. Wacana user hapus+buat ulang akun SambaNova untuk reset kredit — belum diputuskan. Suite 287/287 hijau tiap tahap. Sebelumnya session 162 lanjutan 7 — user komentar "mood-moodan Nemotron ini, ga konsisten" setelah beberapa lanjutan tuning timeout. Eksperimen `think:true` (reasoning dinyalakan, timeout 60s, num_predict 3500) dicoba — hasilnya LEBIH BURUK: 1 sukses tapi 1 gagal TOTAL (Empty response, seluruh token budget habis di reasoning tanpa pernah sampai jawaban), direvert. Dicoba kandidat lain Hermes 3 405B (`?test_hermes=1`, belum pernah dites live) — 3× percobaan semuanya HTTP 429 instan, OpenRouter (pool sama dengan Nemotron) kehabisan kuota hari ini, belum dapat sampel nyata. Keputusan akhir user: SambaNova dikembalikan jadi PRIMARY Call 1 (Nemotron didemote total dari produksi setelah total 5+ sampel latency 7-41s membuktikan reliability-nya tidak bisa diperbaiki dengan tuning timeout — akar masalahnya varian latency sendiri yang liar, bukan soal kurang longgar; kualitas Nemotron sebenarnya bagus tapi tidak cukup kompensasi reliability buruk). Nemotron tidak dihapus, tetap ada di `?test_nemotron=1`/`?test_nemotron_super=1`/`?test_hermes=1` untuk riset lanjutan. `daun_merah_ai.md` diperbarui kembali ke deskripsi SambaNova-primary. Suite 287/287 hijau. Sebelumnya session 162 lanjutan 5 — root cause Nemotron "tidak pernah jalan" ternyata timeout 20s terlalu ketat (bukan garbage) → circuit breaker OPEN 5 menit tiap 2 kegagalan beruntun; dikumpulkan 5 sampel completion time nyata (7s/17.5s/23.9s/29.5s/41.2s), timeout dinaikkan ke 35s. Sebelumnya session 162 lanjutan 4 — user lapor output Nemotron primary kadang rusak (FX/XAU nyatu tanpa marker, bahasa campur, kepotong); root cause: `think:false` native tidak selalu dihonor, `<think>` yang kepotong bocor jadi artikel; fix: `stripThinking()` dikeraskan + validasi format `XAUUSD:` wajib sebelum diterima. Sebelumnya session 162 lanjutan 3 — diagnostik Ollama Cloud `think:false` native BERHASIL di Nemotron Ultra 550B (7s, dulu 4/4 selalu gagal pakai trik prompt `/no_think`), sempat dinaikkan jadi primary. Sebelumnya session 162 lanjutan 1 — bug tampilan headline NEWS "&amp;" literal difix (`decodeXmlEntities()`). Sebelumnya session 162 — fitur baru "Edge per Kondisi Checklist" di tab Jurnal, PLUS backfill 7 trade lama ke production (44/44 item cocok).)
