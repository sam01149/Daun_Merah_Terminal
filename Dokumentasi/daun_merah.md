...
**Konteks:** User minta cek ulang fix Session 254 (larangan narasikan keputusan CB sebagai sudah terjadi). Live-test dengan hit langsung `api/market-digest` production menunjukkan fix S254 BELUM cukup — output baru (generate real-time saat verifikasi, bukan cache basi) MASIH menulis persis kalimat yang sama: *"FOMC tadi malam mempertahankan suku bunga di 3,75% tanpa perubahan..."*. Dicek headline RSS live: tidak ada satupun yang melaporkan hasil keputusan (cuma "Fed Interest Rate Probabilities", artikel probabilitas pra-keputusan).

**Percobaan fix pertama (keliru, sempat dilaporkan ke user sebagai final):** Diagnosis awal mengira ini murni masalah *posisi* — tag `[AKAN RILIS dalam X]` dari `_calEventStatusTag()` sudah benar nempel di `calBlock`, tapi `calBlock` ada di paling bawah user message (setelah ~10 blok data lain), sementara RATE PATH USD ("5bps CUT priced 3m") + FUNDAMENTALS (`Fed Rate: 3.75%`) yang memicu konflasi ada jauh lebih awal. Ditambah blok `cbDecisionAlertBlock` (dihitung di kode) tepat setelah RATE PATH USD supaya sinyal koreksi lebih salient. **Ini di-push duluan (commit `8be9c17`) tapi user langsung koreksi**: kalender ekonomi di app menunjukkan event Fed Interest Rate Decision jatuh Kamis 30 Juli 01:00 WIB, bukan "malam ini 18:00 WIB" seperti yang sempat disebut ke user (itu jam RAW dari XML sebelum dikonversi, salah baca).

**Root cause sebenarnya:** bug di `convertToWIB()` (skrg `toWIB()`) + `parseFFXML()`. XML ForexFactory kasih tanggal `07-29-2026` dan jam raw `6:00pm` (raw = UTC-equivalent, FOMC 2pm ET = 18:00 raw). Konversi ke WIB (+7 jam) menyeberang tengah malam → jam jadi `01:00 WIB` (benar, cocok kalender di app), TAPI dulu cuma jam yang dikonversi (`(hour+7)%24`) sementara **tanggal dipakai apa adanya dari XML** (`2026-07-29`, tidak ikut maju ke `2026-07-30`). Dua field ini (`date`, `time_wib`) jadi merujuk ke DUA hari WIB berbeda. `_calEventStatusTag()` merekonstruksi timestamp dari keduanya dan menghasilkan `2026-07-28T18:00:00Z` — event yang sebenarnya BELUM terjadi (masih ~10 jam lagi dari waktu generate) malah dihitung sebagai **"sudah rilis ~14 jam lalu"** (selisih persis 1 hari, khas bug off-by-one-day). AI menulis "tadi malam" karena mengikuti tag status yang memang salah — bukan murni ngarang tanpa dasar. Bug lama ini (bukan dari perbaikan sesi ini) berarti fix S254 dari awal tidak pernah bisa berhasil untuk event yang jamnya menyeberang tengah malam WIB — termasuk hampir semua rilis Fed/ECB sore-malam ET.

**Fix final (`api/market-digest.js`):** Ganti `convertToWIB(timeStr)` (konversi jam saja) dengan `toWIB(yyyy, mm, dd, timeStr)` — hitung SATU epoch WIB dari tanggal+jam asli sekaligus (`Date.UTC(...) + 7*3600000`), lalu turunkan `date` DAN `time_wib` dari epoch yang sama itu supaya selalu konsisten hari WIB-nya. `parseFFXML()` dan `_calEventStatusTag()`/`cbDecisionAlertBlock` (dari percobaan pertama, tetap dipertahankan sebagai lapisan salience) otomatis benar begitu sumber datanya benar.

**Verifikasi:** `node --check` OK, `npm test` 652/652 hijau (tambah 5 test baru `test/market_digest/market_digest_calendar_wib.test.js` — termasuk kasus persis FOMC 18:00 raw → tanggal maju + jam 01:00 WIB, dan kasus non-rollover sebagai pembanding). Dicek manual: `_calEventStatusTag` untuk event FOMC sekarang menghasilkan `diffH ≈ +10.1` (akan rilis, benar) bukan `-13.9` (dikira sudah rilis, salah) seperti sebelum fix. **Live re-verify pasca-deploy SELESAI (1x hit production, sengaja dihemat karena tiap hit memicu LLM call berbayar):** artikel baru (`generated_at 2026-07-29T07:57:04Z`) membuka dengan *"FOMC akan memutuskan suku bunga dalam 10 jam..."* — forward-looking, sesuai realita (~10 jam sebelum rilis 18:00 UTC), bug hallucination sudah tidak muncul lagi.

**File diubah:** `api/market-digest.js`, `test/market_digest/market_digest_calendar_wib.test.js` (baru).
...
**Temuan emoji tambahan (bukan cuma yang sudah difix S243 awal):** Sambil kerja di area ini ketemu total 7 emoji lagi (`⛔`x3 sebelumnya + `⚠`x4 baru: 2 di warning sizing, 1 di info box kontra buy/sell limit, 1 di thesis-alert-header) — dihapus sambil jalan. Grep menyeluruh `⚠` saja (tanpa hitung simbol lain) menemukan **~35 sisa** tersebar sistematis di disclaimer AI/warning badge/stale-indicator — bukan kecelakaan kecil, ini konvensi lama yang perlu keputusan sadar (ganti ke teks polos atau ikon SVG?) sebelum dibongkar semua. Detail & syarat lanjut di `daun_merah_progress.md`.
...
4. **`icon.svg` TIDAK dipindah** — tetap di root karena user cuma minta rapikan `.png` (icon.svg format SVG, dan sudah dipakai langsung sebagai favicon + logo header, tidak perlu ganti path).
...
3. `index.html` [MODIFY] — Mengubah navigasi tombol pada panel Analisa Pair dan Kartu AI Thesis dari "KE SIZING CALC" ke "KE CHECKLIST" (`analisaGoToChecklist` & `thesisGoToChecklist`). Menghapus duplikasi tombol di Checklist desktop. Memperbaiki *formatting paradox* subtitle verdict `ckConflict`. Menyambungkan passing data setup (`_storePendingSizingSetup`) saat navigasi Analisa/Thesis -> Checklist -> Sizing Calculator (`ckGoToSizing`). Merestrukturisasi tombol aksi di hasil Sizing Calculator (`calcSizing`): mengganti tombol "LANJUT KE CHECKLIST" dengan "Entry MT5" (`szShowMt5Modal`) & "→ Buat Jurnal" (`ckPrefillJurnal`) serta menambahkan helper `_syncChecklistForPair()` agar state checklist otomatis tersinkronisasi tanpa memaksa pengguna kembali ke Checklist. Bump `APP_VERSION` ke `2026.07.22.5`.
4. `npm test` dijalankan dan terverifikasi 100% hijau (555/555 pass).
