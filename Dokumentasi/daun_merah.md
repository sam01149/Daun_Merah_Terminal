...
> **Last updated:** 2026-07-13 (session 162 lanjutan 4 — user lapor output Nemotron primary kadang rusak (FX/XAU nyatu tanpa marker, bahasa campur, kepotong) 3× berturut-turut di production; root cause: `think:false` native tidak selalu dihonor Nemotron 3 Ultra, kalau `<think>` kebuka tapi kepotong sebelum `num_predict` habis, `stripThinking()` lama gagal strip dan reasoning mentah bocor jadi artikel; fix: `stripThinking()` dikeraskan (buang unterminated `<think>` alih-alih leak), blok penerimaan Nemotron primary sekarang wajib validasi `raw.includes('XAUUSD:')` sebelum diterima (gagal → `cb.onFailure` + fallback ke SambaNova di request yang sama, bukan serve rusak), plus log `done_reason=length` di `callOllama()` menyamakan pola `aiCall()`. 2 test baru, suite 287/287 hijau. Sebelumnya session 162 lanjutan 3 — diagnostik Ollama Cloud dilanjutkan dengan parameter native `think:false` (top-level body `/api/chat`, beda dari trik prompt `/no_think` yang selalu gagal sebelumnya): retest `nemotron-3-nano:30b-cloud` (`?test_ollama=1`) BERHASIL (dulu HTTP 200 tapi kosong, sekarang lengkap 8,1s), lalu mekanisme sama dicoba ke Nemotron Ultra 550B (`?test_nemotron=1`, dulu 4/4 selalu gagal) — BERHASIL juga, 7s, artikel lengkap kualitas bagus. Baru 1× sampel Ultra, belum cukup untuk klaim siap produksi — rencana Hugging Face (model ringan gratis) dibatalkan setelah riset menunjukkan free tier HF dibatasi <10B parameter dan tidak ada lagi model gratis setara DeepSeek V3.2 di provider manapun yang terverifikasi (OpenRouter sudah hapus semua varian DeepSeek gratis Juli 2026). Sebelumnya session 162 lanjutan 1 — bug tampilan headline NEWS ditemukan & difix: judul yang mengandung "&" (mis. "S&P 500") tampil literal sebagai "S&amp;P 500" di UI. Root cause: RSS `<title>` sudah XML-escaped di feed asal ("&amp;" untuk "&" literal), tapi 3 parser duplikat (`parseRSS` di index.html, `parseItems` di sw.js, `parseRSSItems` di api/feeds.js) cuma extract teks mentah tanpa decode entity — `escHtml()` di render time lalu meng-escape "&" yang tersisa itu SEKALI LAGI, browser render "&amp;" literal. Fix: `decodeXmlEntities()` baru (named + numerik desimal/hex) ditambahkan ke title extraction di ketiga tempat (lockstep manual, tidak bisa share module lintas SW/main-thread/backend). Sebelumnya session 162 — fitur baru "Edge per Kondisi Checklist" di tab Jurnal: menjawab "kondisi checklist mana yang beneran ngasih edge?" dari data trade closed asli, terinspirasi konsep factor analysis (Alphalens) tapi diterapkan ke sinyal diskresioner sendiri, bukan backtest sistematis; PLUS backfill ke production (`scripts/backfill_checklist_snapshot.js`) — 7 trade lama sebelum fitur ini ada ternyata bisa direkonstruksi checklist-nya dari baris ✅/⬜ yang sudah lama tersimpan di `thesis_text`, semua 7 berhasil 44/44 item cocok — lihat changelog di bawah untuk detail. Sebelumnya session 161 lanjutan 3 — root cause NEWS gagal fetch khusus di HP ditemukan: timeout client-side `fetchRSS()` (12s) lebih pendek dari `maxDuration` server `api/feeds.js` (20s), jadi di jaringan seluler yang RTT-nya lebih tinggi/variatif request di-abort duluan sebelum server sempat balas walau server masih dalam batas wajar — dinaikkan ke 22s di `index.html` & `sw.js` (lockstep). Sebelumnya session 161 lanjutan 2 — bug volume 0 di kartu TA Teknikal ditemukan & difix (`current.volume || null` salah nol-kan volume 0 yang valid), lalu user putuskan Volume di TA Teknikal dihapus total saja alih-alih terus dipatch — field `current_volume`/`volume_sma_20`/`volume_status` dibuang dari `api/correlations.js`, kartu Volume dibuang dari `renderTaPanel`, `fmtVol()` dead code ikut dibersihkan. Sebelumnya session 161 lanjutan 1 — root cause Option Expiries 502 ditemukan LIVE via curl langsung ke vendor: bug retensi Redis 4 jam yang sama dengan window freshness bikin stale-cache fallback kehilangan data begitu key expire, dinaikkan ke 48 jam; investinglive.com dikonfirmasi feed kosong 5 hari (vendor issue, bukan blocking); pesan error mentah "HTTP 502" di UI diganti kalimat tenang; layout tab Analisa desktop dilebarkan + kartu jadi grid 2×2 (dulu kolom baca 680px "manjang banget"); korelasi Gold teks "berubah dari biasa" diperjelas jadi "biasanya memiliki korelasi yang {kata}"; volume XAU di Analisa dikonfirmasi data asli (Yahoo GC=F), tidak dihapus. Sebelumnya session 161 — rangkaian 10 revisi UX dari review manual user: cache berita client-side biar tidak "gagal fetch" saat FinancialJuice down, Fundamental mobile collapse-by-default, kalendar default HARI INI SAJA + fallback ForexFactory dihapus, filter sumber ARTIKEL jadi dropdown checklist, checklist auto-check naik dari 5→6 kondisi (+ retail sentiment kontrarian) & emoji petir dihapus, Jurnal thesis dapat toggle collapse, Volume Teknikal disembunyikan utk FX OTC, korelasi/anomali disederhanakan jadi teks tanpa bar chart; sebelumnya session 160 — navbar jadi rail kiri ikon+label di desktop/tablet + header full-width fixed permanen di atasnya, gaya terminal profesional; hapus status-pill header, panel "Distribusi Berita", dan tabel candle mentah di tab Analisa karena noise/tidak informatif)
