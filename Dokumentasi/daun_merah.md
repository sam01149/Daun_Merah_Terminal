...
> **Last updated:** 2026-07-13 (session 162 lanjutan 5 — user lapor "Nemotron ga pernah bikin ringkasan, SambaNova terus" setelah fix lanjutan 4; investigasi log: fix lanjutan 4 sudah bekerja (0 `bad_format`), tapi Nemotron primary konsisten timeout di batas lama 20s → circuit breaker OPEN 5 menit tiap 2 kegagalan beruntun, kelihatan seperti "tidak pernah jalan". Dikumpulkan 5 sampel completion time nyata (7s, 17.5s, 23.9s, 29.5s, 41.2s — rata-rata 23,8s tapi variasi 6x lipat, konsisten dugaan lama resource-contention tier gratis). Cek kualitas: 3 artikel Nemotron 0 pelanggaran frasa terlarang vs SambaNova (yang jalan production hari sama) ketahuan leak "dapat memberikan"/"di tengah" 2×. User putuskan: tetap Nemotron primary untuk SEMUA request (bukan cuma cron) demi kualitas, timeout dinaikkan 20s→35s (aman terhadap guard `CALL3_BUDGET_MS` yang sudah ada). Suite 287/287 hijau. Sebelumnya session 162 lanjutan 4 — user lapor output Nemotron primary kadang rusak (FX/XAU nyatu tanpa marker, bahasa campur, kepotong); root cause: `think:false` native tidak selalu dihonor Nemotron 3 Ultra, kalau `<think>` kebuka tapi kepotong sebelum `num_predict` habis, `stripThinking()` lama gagal strip dan reasoning mentah bocor jadi artikel; fix: `stripThinking()` dikeraskan (buang unterminated `<think>` alih-alih leak), blok penerimaan Nemotron primary wajib validasi `raw.includes('XAUUSD:')` sebelum diterima (gagal → `cb.onFailure` + fallback ke SambaNova di request yang sama), plus log `done_reason=length` di `callOllama()`. Sebelumnya session 162 lanjutan 3 — diagnostik Ollama Cloud dilanjutkan dengan parameter native `think:false` (top-level body `/api/chat`, beda dari trik prompt `/no_think` yang selalu gagal sebelumnya): retest `nemotron-3-nano:30b-cloud` (`?test_ollama=1`) BERHASIL (dulu HTTP 200 tapi kosong, sekarang lengkap 8,1s), lalu mekanisme sama dicoba ke Nemotron Ultra 550B (`?test_nemotron=1`, dulu 4/4 selalu gagal) — BERHASIL juga, 7s, artikel lengkap kualitas bagus. Sebelumnya session 162 lanjutan 1 — bug tampilan headline NEWS ditemukan & difix: judul yang mengandung "&" (mis. "S&P 500") tampil literal sebagai "S&amp;P 500" di UI karena double-escape (`decodeXmlEntities()` baru ditambahkan ke 3 parser). Sebelumnya session 162 — fitur baru "Edge per Kondisi Checklist" di tab Jurnal, PLUS backfill 7 trade lama ke production (44/44 item cocok). Sebelumnya session 161 lanjutan 3 — root cause NEWS gagal fetch khusus di HP: timeout client-side (12s) lebih pendek dari server (20s), dinaikkan ke 22s lockstep.)
