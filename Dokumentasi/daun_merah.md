...
3. `index.html` [MODIFY] — Mengubah navigasi tombol pada panel Analisa Pair dan Kartu AI Thesis dari "KE SIZING CALC / Mulai ke Sizing Calc →" menjadi "KE CHECKLIST / Mulai ke Checklist →" (`analisaGoToChecklist` & `thesisGoToChecklist`). Menghapus duplikasi 2 tombol "Buat Jurnal / Lanjut ke Sizing" di bagian atas tampilan desktop Checklist dengan membatasi tombol mobile via CSS `@media (max-width: 600px)` & `classList.toggle('visible')` sehingga hanya tombol sidebar kanan-bawah yang tampil di desktop. Bump `APP_VERSION` ke `2026.07.22.2`.
