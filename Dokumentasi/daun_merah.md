...
> **Last updated:** 2026-07-13 (session 164 — user cek provider pihak-ketiga `tokenreply.com` sebagai kandidat AI: **DITOLAK** — proxy tidak resmi, domain baru ~2 bulan, semua model $0.00/request, nama model mencurigakan (codename arena-testing seperti "big-pickle"/"north-mini-code"), tidak direkomendasikan/tidak dipakai. Sambil diskusi rantai provider Call 1/2/3, user tunjukkan log live gagal total: primary OpenRouter gpt-oss-120b:free timeout 15010ms, fallback SambaNova akun 2 HTTP 402, fallback Groq HTTP 413 — semua tier gagal. **Fix:** primary Call 1 (live/on-demand) diganti dari OpenRouter ke **Cerebras `gpt-oss-120b`** (endpoint/key sama dgn admin.js/journal.js sejak session 145, ~3000 tok/s, context 128K) — fallback chain (SambaNova akun 2 → Groq) tidak berubah, cuma provider primary yang diganti. `OPENROUTER_MODEL` (konstanta lama) dihapus, `CEREBRAS_MODEL_GPTOSS`/`CB_CEREBRAS_GPTOSS` ditambahkan. Belum ditindaklanjuti (di luar scope): HTTP 402 SambaNova akun 2 (kemungkinan billing, lihat [[project-sambanova-billing-risk]]) dan HTTP 413 Groq (indikasi prompt kepanjangan). `node --check` bersih; belum dites live end-to-end (hemat kuota). Sebelumnya session 163 — Z.ai GLM 4.7 via Cerebras dites & **DITOLAK** (context tier Preview cuma 8192 token < prompt Call 1 ~13K); ditemukan akun SambaNova (Call 1, akun 2) ternyata paid Developer Tier bukan free (kredit signup habis); gpt-oss-120b (OpenRouter) sempat dipromosikan jadi primary Call 1 live, Nemotron Ultra (Ollama) jadi primary KHUSUS 3 cron session-open (hasil ke cache, latency liar tidak dirasakan live user); guard `CALL1_HARD_BUDGET_MS` (48s) ditambahkan supaya cascade gagal berantai tidak mendorong Call 1 tembus `maxDuration` 60s Vercel. Suite 287/287 hijau tiap sesi.)
