...
**Follow-up terbuka (belum dieksekusi, masih diskusi):** User tanya soal font — 335 dari 354 deklarasi font di app pakai `'DM Mono'` (monospace) vs 48 `'Syne'` (display) vs cuma 7 lari ke `system-ui`. Sama seperti warna, ini juga di-flag Gemini sebagai titik kemiripan WarWatch ("sama-sama monospace tactical HUD" — DM Mono/Syne vs JetBrains Mono). Rekomendasi yang diajukan (belum disetujui/dieksekusi): pertahankan DM Mono untuk data/angka/label (fungsional buat rata kolom), tapi perluas pola `system-ui` yang sudah dipakai `.riset-title` ke area teks panjang lain (deskripsi berita, thesis AI) yang saat ini masih monospace — tanpa nambah font baru. Kalau dilanjutkan sesi depan, mulai dari sini.
