...
5. `api/_pair_context.js` — `STRUCTURAL_PROFILES` **SENGAJA tidak diisi** untuk CHF/JPY (beda dari AUD/NZD & EUR/GBP) — riset korelasi menemukan mekanisme "dua safe-haven saling menetralkan" TIDAK terbukti untuk CHF/JPY (SNB & BOJ pendorong independen, bukan searah macam RBA-RBNZ), jadi pair ini kemungkinan macro-driven/trending, bukan mean-reverting. Nempel catatan struktural yang salah asumsi lebih berbahaya daripada tidak ada sama sekali.
