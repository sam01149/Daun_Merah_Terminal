...
## Changelog Session 303 (2026-08-10) — Gate Baru `makro_conflict` + Commentary AI Disimpan ke Log

**Konteks:** Lanjutan audit CHF/JPY sesi yang sama dengan [[project-currency-strength-cot-source-leakage-bug]] (Session 302). User audit satu setup CHF/JPY bullish dengan `makro_alignment:"konflik"` (JPY diprediksi menguat, berlawanan arah bias bullish) TAPI tetap tersimpan `pending` dengan entry/SL/TP lengkap — user tanya "itu harusnya gaboleh atau gimana??" setelah memverifikasi pola yang sama terjadi di pair lain juga (bukan kejadian tunggal). Ditelusuri: instruksi prompt (`api/admin.js`, `entryZoneInstr`) SUDAH sejak awal minta AI meng-null-kan `entry_zone/sl/tp/entry_basis` kalau `makro_alignment` adalah "konflik" — tapi ini murni permintaan teks, TIDAK PERNAH di-enforce kode (beda dari sanity-check RR<1 yang memang code-enforced). Kemungkinan besar root cause: field `entry_zone/sl/tp` diminta LEBIH DULU dari `makro_alignment/conflict` di skema JSON yang sama, jadi untuk model non-reasoning ("flash"), level trade sudah ter-commit sebelum model "memutuskan" konfliknya sendiri di field belakangnya.

Terpisah, user juga menanyakan kenapa `bias` bisa tetap "bullish" walau macro conflict — dijelaskan: itu memang BUKAN bug, `bias` didasarkan HANYA pada struktur teknikal (Daily+H4+BOS), independen dari `makro_alignment` (perbandingan terpisah) — desain yang disengaja, bukan cacat. TAPI penjelasan "kenapa teknikalnya masih dianggap kuat" itu ada di paragraf commentary AI, dan ternyata **field itu tidak pernah disimpan ke `setup_log_auto:v1` sama sekali** (`buildNewSetupEntry` cuma menyimpan field terstruktur) — untuk setup auto (cron, tanpa penonton live), penjelasan itu hilang permanen begitu response API dibalas.

**Perubahan (`api/admin.js`):**
1. **Gate baru `makro_conflict`** — ditambahkan ke chain `autoGuardConsidered` (persis pola `correlation_cap`/`drawdown_circuit_breaker`, DI BAWAH Gate A sehingga Gate A tidak perlu dipanggil kalau gate ini sudah menahan — hemat 1 call AI). Kalau `structured.makro_alignment` FINAL (setelah Sistem Hakim + Cek Kontradiksi Session 301 selesai mengoreksi) = `"konflik"`, kandidat `source:auto` TIDAK PERNAH jadi entri live (`pending`/`open`) — tersimpan sebagai `canceled`/`canceled_reason:"gate_makro_conflict"` DENGAN level asli tetap direkam (bukan di-null-kan di titik parsing) supaya `_evaluateCanceledGhost` bisa mengevaluasi counterfactual-nya (apakah gate ini benar menahan atau membuang kandidat yang sebenarnya menang) — `gate_makro_conflict` ditambahkan ke `GHOST_TRACKED_CANCEL_REASONS`. Sengaja TIDAK memutasi `structured.entry_zone/sl/tp` langsung (percobaan pertama begitu, DIBATALKAN — itu bikin gate `if (structured?.entry_zone...)` di atasnya jadi false, seluruh blok logging termasuk ghost-tracking ikut ter-skip, kandidat hilang TANPA jejak sama sekali). **HANYA `source:auto`** (pola sama semua Gate D/B/A) — manual (`ohlcvAnalyzeHandler` tanpa `auto=1`) TIDAK disentuh, `makro_alignment`/entry tetap tampil apa adanya di response API, keputusan tetap di tangan pembaca.
2. **`commentary` ditambah ke `buildNewSetupEntry`** — narasi lengkap 5-paragraf AI (termasuk penjelasan integrasi teknikal-vs-makro di paragraf 4) sekarang disimpan apa adanya (nullable, tanpa cap panjang tambahan — sudah dibatasi alami oleh instruksi prompt). Berlaku untuk `setup_log:v1` (manual) MAUPUN `setup_log_auto:v1` (auto) karena satu closure yang sama.
3. Registry (`KEY_REGISTRY`) — tambah entri `auto_guard_stats:makro_conflict`, update komentar invarian `considered = saved + makro_conflict + correlation_cap + drawdown_circuit_breaker + critic_veto`.

**Perubahan (`dev-auto-entry.html`):** helper baru `fldLong()` (beda dari `fld()` — full-width, `white-space:pre-wrap` supaya paragraf commentary tidak dipadatkan jadi satu baris tanpa jeda), dipasang sebagai baris "Analisa Lengkap (AI)" di grup Ringkasan `buildSetupDetail()`. Label `GATE_GHOST_LABELS.gate_makro_conflict` + hint card ghost-tracking diupdate menyebut gate baru.

**Test:** `test/admin/gate_makro_conflict.test.js` (baru, 3 test: auto+konflik → canceled + Gate A dilewati + level asli terekam; manual+konflik → tetap pending, perilaku tidak berubah; `GHOST_TRACKED_CANCEL_REASONS` mencakup reason baru), + 1 assertion baru di `test/admin/pair_context_prompt.test.js` (commentary tersimpan ke `setup_log:v1`). Test lama (`gate_a_race.test.js`, `alignment_contradiction.test.js`) sempat regresi 2 kasus saat percobaan pertama (structured di-null langsung) — hilang setelah pindah ke pola Gate. `npm test` 929/929 hijau.

**Cakupan/limitasi (WAJIB dibaca sebelum audit lanjutan, didokumentasikan di `audit_workflow.md` §3a):** gate ini dicek SETELAH logika "refine stale pending" yang sudah ada (kandidat baru symbol+bias SAMA dengan pending lama meng-update levelnya in-place TANPA cek gate dulu) — kalau kandidat yang dipakai refine itu KEMUDIAN ditolak gate `makro_conflict` (atau `correlation_cap`/`drawdown`, limitasi PRE-EXISTING bukan baru sesi ini), pending lama sempat ter-update ke level yang "seharusnya" ditolak sebelum entri ghost-canceled terpisah ditambahkan. Skenario sempit, TIDAK difix sesi ini (perlu restrukturisasi urutan refine-vs-gate, di luar cakupan audit ini) — dicatat sebagai known-limitation, bukan diam-diam diabaikan.
