...
## Changelog Session 322 (2026-08-18) — PLAN Z: Kandidat SL/TP/Invalidasi Deterministik

**Konteks:** eksekusi `plan.md` folder ini §PLAN Z (asal temuan S317 lanjutan 2: dua run analisa XAU/USD dengan input identik, `temperature: 0`, menghasilkan SL beda 16 poin dan `invalidation_trigger` beda tipe sepenuhnya — risiko per trade 1R sebagian ditentukan kebetulan generate, bukan analisa). Memperluas pola `_confluenceZones`/`entryZoneInstr` yang sudah terbukti menstabilkan `entry_zone` ke dua field yang masih bebas dikarang AI: `sl` dan `tp`.

**Implementasi:**
1. **Modul murni baru `api/_levels.js`** (`computeLevelCandidates`) — menerima `zones` (output `_confluenceZones` yang SUDAH dihitung admin.js, bukan pipeline ekstraksi kedua) + ATR + presisi desimal, menghasilkan kandidat SL/TP untuk KEDUA arah bias sekaligus (bias belum diketahui saat prompt dibangun, pola sama `entryZoneInstr`). Per sisi: SL = semua zona KECUALI yang terdekat ke Now (diasumsikan itu pilihan entry_zone AI) + SATU kandidat sintetis "buffer ATR di luar struktur terjauh" (buffer sama persis `slBufferMult` yang sudah ada: 1x ATR XAU, 0.5x ATR FX) — menjamin SL selalu ada walau confluence tipis. TP = semua zona di sisi profit, DIFILTER supaya kombinasi RR<1 (dibandingkan SL terdekat, skenario terbaik) tidak pernah ditawarkan. `time_horizon_days` sengaja TIDAK dipakai menyaring TP jauh (field itu dihasilkan AI di respons yang sama, tidak tersedia saat prompt dibangun).
2. **`api/admin.js`** — `_formatLevelCandidatesBlock` (render `[KANDIDAT SL/TP]`, gaya sama `_formatConfluenceBlock`) disisipkan ke prompt dekat `[ZONA KONFLUENSI]`. `slInstr`/`tpInstr` sekarang TIGA varian (kandidat > confBlock lama > generic lama) dengan klausa fallback eksplisit untuk kasus levelCandidates cuma punya satu arah (umum terjadi kalau filter RR membuang semua TP satu sisi). Instruksi `invalidation_trigger` ditambah satu kalimat: level sebaiknya sama dengan salah satu kandidat.
3. **Snap-atau-tolak setelah parsing** — kalau AI menulis angka dalam toleransi cluster zona (`zones.tolerance`, granularitas sama yang menyatukan level berdekatan di `_confluenceZones`) dari kandidat, disamakan presisinya (snap). Kalau menyimpang jauh dari SEMUA kandidat untuk bias yang dipilih, level trio (`entry_zone`/`sl`/`tp`) di-null-kan — perlakuan SAMA seperti sanity-check RR/direction yang sudah ada (bukan mekanisme penolakan baru). `levelCandidates` null (data minim) atau bias netral/mixed → langkah ini dilewati total, fallback ke instruksi lama.
4. **Epoch baru `POLICY_EPOCHS` v28** (`kind: 'policy'`, `impact: 'levels'`) — perilaku lama (AI bebas menentukan SL/TP) memang desain awal yang disengaja, bukan kerusakan, jadi ini `policy` bukan `fix`. Laporan performa lintas waktu (n≥100 Plan U, dst) WAJIB memisah `policy_v`/`policy_v_est` sebelum vs sesudah v28.

**Bonus temuan saat menambah epoch v28:** v26 & v27 di `POLICY_EPOCHS` ternyata punya `from` yang TIDAK cocok waktu commit git log asli — v26 salah salin waktu lokal WIB tanpa konversi UTC (selisih 7 jam), v27 angka bulat yang tidak cocok waktu manapun (selisih ~10 jam). Dikoreksi ke waktu commit asli (`git log --format=%aI`) SETELAH konfirmasi eksplisit user (soal apakah ini melanggar aturan "jangan edit epoch lama" — user memutuskan koreksi metadata yang salah ketik BEDA dari mengubah aturan main yang berlaku). v1-v25 BELUM diaudit ulang — diparkir di `progress.md` folder ini.

**Test:** 17 unit test baru (`test/api/_levels.test.js` — determinisme, SL/TP di sisi benar untuk bullish&bearish, RR<1 tersaring, data minim → null bukan crash, format tetap terbaca regex existing, dll; 2 test baru `_formatLevelCandidatesBlock` di `test/admin/ta_struct.test.js`). `npm test` 1050/1050 hijau.

**Verifikasi live (production, jalur publik tanpa auth khusus — sama seperti user asli klik "Analisa AI"):**
- 3x panggil `?action=ohlcv_analyze&symbol=GC=F&test_deepseek=1` berturut-turut: **SL & TP identik PERSIS di 3/3 run** (sl=4488.33, tp=4120.04, entry_zone=4434.62 — semua match S1/T1 di `[KANDIDAT SL/TP]`, tanpa perlu snap), `invalidation_trigger.type`/`level`/`direction` identik di 3/3 (`price_level`/4449.7/`above`), cuma `timeframe` yang sedikit beda (1d vs 4h — field ini tidak dikonstrain kandidat). Jauh melebihi ambang lolos plan ("SL & TP identik minimal 2/3, invalidation_trigger.type identik 3/3").
- Sanity-check jalur manual publik sungguhan (EUR/USD, TANPA `test_deepseek`/`auto`): HTTP 200, structured lengkap (bias/entry/sl/tp/invalidation_trigger/RR=1.86 semua konsisten arah), commentary 2778 karakter normal — jalur publik tidak rusak oleh perubahan prompt.
- Dibandingkan bug asal (S317 lanjutan 2: SL beda 16 poin, invalidation_trigger beda TIPE sepenuhnya antar 2 run) — sekarang identik total di 3 run berturut-turut untuk pair yang sama.

**Verifikasi susulan (bukan penghalang, dipantau berkala):** `consistency_log:v1` (`levels_within_tolerance`) belum dicek beberapa hari pasca-deploy — bukan kriteria wajib merge, lihat plan.md riwayat.

## Changelog Session 321 (2026-08-18) — Panel Detail `canceled` Diam-Diam Menyembunyikan Sebab Aslinya

**Konteks:** user tanya kenapa dua setup (`AUDNZD=X:1787040925559`, `CHFJPY=X:1787040937229`) berstatus `canceled` padahal tidak ada sinyal baru maupun posisi terbuka lain untuk pair yang sama — dua penyebab yang justru disebutkan panel detail (`dev-auto-entry.html`) itu sendiri. Audit menelusuri ke `STATUS_EXPLAIN.canceled`/`buildStatusNarrative`: teks itu SATU kalimat generik hardcode untuk SEMUA status `canceled`, tidak pernah membaca field `canceled_reason` yang sebenarnya sudah dikirim server (`_statsPayloadFromLog` -> `symbols[k].history`) sejak `canceled_reason` mulai direkam (2026-08-08).

**Akar masalah:** `canceled` di `api/admin.js` punya 2 mekanisme yang beda arti sama sekali — (1) `bias_flip`: pending LAMA dibatalkan karena AI generate sinyal BARU berbias berlawanan untuk pair sama (Flip Guard non-whipsaw); (2) `gate_correlation_cap`/`gate_drawdown_circuit_breaker`/`gate_critic_veto`: kandidat BARU itu sendiri ditahan salah satu dari 3 gerbang risiko (Gate D/B/A) SEBELUM sempat jadi `pending` sama sekali — tidak pernah ada "sinyal baru" atau "posisi terbuka" yang terlibat. Kedua setup yang ditanya user cocok pola #2 (entri terbaru untuk pair itu sendiri yang `canceled`, bukan entri lama yang digantikan) — AUD/NZD malah tidak punya pair berkorelasi di `CORRELATED_PAIRS` sama sekali, jadi `gate_correlation_cap` mustahil untuknya, menyisakan drawdown circuit breaker atau veto Kritikus sebagai sebab sebenarnya.

**Fix:** `dev-auto-entry.html` — tambah `CANCELED_REASON_EXPLAIN` (map 4 `canceled_reason` ke narasi spesifik) dan cabang eksplisit `s.status === 'canceled'` di `buildStatusNarrative` yang membaca `canceled_reason` entri ybs (fallback ke catatan "tidak tercatat" untuk entri pra-2026-08-08). `STATUS_EXPLAIN.canceled` (dipakai field "Penjelasan Status") diperbarui jadi definisi kategori yang akurat, tidak lagi menyiratkan 2 sebab yang cuma kadang benar.

**Tidak diubah:** tidak ada perubahan backend/API — `canceled_reason` sudah terkirim di payload sebelumnya, murni gap render di frontend. Detail keberatan spesifik AI Kritikus (`critic.objections[0].reason`) TETAP tidak tersimpan ke entri (cuma di server log) — di luar scope fix ini, dicatat sebagai keterbatasan di teks `gate_critic_veto`.

**Test:** `npm test` 1048/1048 hijau (perubahan murni string/markup, tidak menyentuh kode teruji).
