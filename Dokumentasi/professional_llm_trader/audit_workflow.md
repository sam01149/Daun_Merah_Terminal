...
**RESOLVED Session 303 (2026-08-10).** Sebelumnya `Makro Alignment (mentah) = "konflik"` **seharusnya** membuat `entry_zone/sl/tp/entry_basis` di-null-kan oleh AI sendiri (instruksi eksplisit ada di prompt, `api/admin.js` `entryZoneInstr`) — TAPI itu **prompt-only, TIDAK ada enforcement kode** (beda dari sanity-check RR<1 yang memang code-enforced), kemungkinan besar karena `entry_zone/sl/tp` diminta LEBIH DULU dari `makro_alignment` di skema JSON, jadi levelnya sudah ter-commit sebelum model "memutuskan" konflik. Fix: gate baru `makro_conflict` (`autoGuardConsidered`, pola sama `correlation_cap`/`drawdown_circuit_breaker`) — kandidat `source:auto` dengan `makro_alignment` FINAL (setelah Sistem Hakim + Cek Kontradiksi) = "konflik" TIDAK PERNAH jadi entri live, tersimpan sebagai `canceled`/`canceled_reason:'gate_makro_conflict'` dengan level ASLI tetap direkam untuk ghost-tracking (`GHOST_TRACKED_CANCEL_REASONS`). **HANYA berlaku `source:auto`** — manual TIDAK digate (keputusan tetap di tangan pembaca), `makro_alignment`/entry tetap tampil apa adanya di response API manual.

**Limitasi yang MASIH ADA (belum difix, diketahui sejak awal):** gate ini dicek SETELAH logika "refine stale pending" (kandidat baru dengan symbol+bias SAMA seperti pending lama meng-update level pending itu in-place, TANPA cek gate dulu) — kalau kandidat yang di-refine-kan itu KEMUDIAN ditolak gate `makro_conflict` (atau `correlation_cap`/`drawdown`, limitasi ini bukan baru), pending lama sempat ter-update ke level yang "seharusnya" ditolak, sebelum entri ghost-canceled terpisah ditambahkan untuk kandidat barunya. Skenario sempit (butuh stale pending symbol+bias sama PERSIS dengan kandidat yang ditolak), tapi kalau nemu kasus nyata saat audit — cross-check `entry_zone` pending itu terhadap kandidat ghost-canceled dengan `ts` berdekatan, bukan otomatis anggap gate gagal total.
...
| 303 (`changelog.md` folder ini) | `makro_alignment:"konflik"` tidak menghasilkan entry_zone null — instruksi prompt tidak code-enforced (lihat §3a) | Gate baru `makro_conflict` (auto-only, ghost-tracked, pola sama correlation_cap/drawdown) |
