...
`Makro Alignment (mentah) = "konflik"` **seharusnya** membuat `entry_zone/sl/tp/entry_basis` di-null-kan oleh AI sendiri (instruksi eksplisit ada di prompt, `api/admin.js` `entryZoneInstr`) — TAPI ini **prompt-only, TIDAK ada enforcement kode** (beda dari sanity-check RR<1 yang memang code-enforced). Root cause terdokumentasi: field `entry_zone/sl/tp` muncul LEBIH DULU dari `makro_alignment` di skema JSON yang diminta ke model (`bias→entry_zone→...→makro_alignment→conflict`) — untuk model non-reasoning ("flash"), token entry sudah ter-commit SEBELUM model "memutuskan" alignment-nya sendiri di field belakangnya. **Kalau ketemu kasus ini (konflik tapi entry tetap terisi):** BUKAN otomatis bug baru — ini gap arsitektur yang sudah diketahui (didiskusikan Session 302, belum diputuskan fix-nya: reorder skema JSON vs enforcement kode vs biarkan Gate A/label confidence sebagai sinyal). Catat sebagai observasi berulang di `riset.md` folder ini kalau ingin menambah bukti untuk keputusan itu; jangan buka ulang diskusi dari nol.
...
| 302 (§3a di atas, BELUM diputuskan) | `makro_alignment:"konflik"` tidak selalu menghasilkan entry_zone null — instruksi prompt tidak code-enforced, kemungkinan root cause urutan field JSON | Belum ada — didiskusikan, opsi: reorder skema / enforcement kode / biarkan Gate A+label confidence sebagai sinyal |
