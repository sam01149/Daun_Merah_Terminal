...
| Parameter Trade | Risk/Reward, Horizon, **Model**, **Versi Kebijakan** | Cek model AI yang dipakai (beberapa model lebih rawan salah nalar dari yang lain). **Versi Kebijakan** (2026-08-18) = aturan main auto-entry yang berlaku saat setup itu lahir (`policy_v` kalau direkam, `policy_v_est` kalau hasil rekonstruksi dari `ts`) — WAJIB dilihat sebelum membandingkan setup lintas tanggal, karena gate/pair/prompt sudah berubah puluhan kali; daftar versi ada di `POLICY_EPOCHS` (`api/_auto_entry_guard.js`) & ikut dikirim di payload scope=auto |
...
- **Pola berulang atau cacat mekanisme jelas (bukan cuma hasil buruk sesekali):** ini bugfix — ikuti alur normal proyek: perbaiki kode, tambah test regresi, `npm test` 100% hijau, tulis changelog. **Routing dokumentasi (DIUBAH 2026-08-18, keputusan user — `ATURAN.md` §2):** SEMUA temuan audit auto-entry → `changelog.md` FOLDER INI, termasuk yang perbaikannya ada di logika/prompt yang dipakai bersama manual+auto (mis. `ohlcvAnalyzeHandler`, `_formatFundamentalBlock`, `_pair_context.js`). Aturan lama mengarahkan kasus bersama itu ke `daun_merah.md` (preseden Session 301/302) — DICABUT. Satu-satunya kewajiban tambahan: kalau perubahannya mengubah perilaku yang dilihat pengguna publik, tulis SATU baris penunjuk di `daun_merah.md` yang merujuk entri lengkap di sini (penunjuk, bukan salinan).
...
| 318 lanjutan / 319 (SUDAH DIFIX, `changelog.md` folder ini Session 319) | Gate D (`correlation_cap`) cuma menghitung partner ber-`status:'open'`, sehingga dua setup korelatif yang sama-sama masih `pending` (limit order belum kesentuh) lolos cap dan bisa terisi bersamaan — padahal `pending` justru state terlama di sistem berbasis limit order zona konfluensi. Kelas bug: **cakupan gate vs siklus hidup setup** (bukan salah hitung korelasi) — saat mengaudit gate lain, cek juga status apa saja yang dia lihat | `EXPOSURE_BINDING_STATUSES` (`open` + `pending`) dipakai Gate D; sengaja TANPA ambang umur pending (hindari angka tebakan baru), biaya/manfaat diukur lewat ghost `gate_correlation_cap` |
