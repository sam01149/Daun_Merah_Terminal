...
| 10 | **Translate NEWS ke Bahasa Indonesia** (S272, 2026-08-02 — toggle 🇮🇩/🇬🇧 di tab NEWS, field tambahan `title_id`/`desc_id`, teks Inggris asli TETAP dipertahankan utuh untuk `newscat.js`/filter TEK/push notif; sempat dicoba pindah ke SambaNova akun 2 sesi sama hari itu supaya lebih cepat — TERBUKTI SALAH lewat verifikasi live: akun itu dipakai bersama 3 fitur lain, circuit breaker trip berulang, headline malah sering tidak diterjemahkan berjam-jam. **Balik ke Gemini + desain BATCH** (satu panggilan API menerjemahkan sampai 20 headline sekaligus, bukan 1 headline/panggilan) — jumlah panggilan nyaris tidak pernah dekat limit 10 RPM walau breaking news deras) | Toggle bahasa (bukan tombol generate — otomatis di background) | Ya — tiap cache RSS di-refill (~50-60 detik) & tiap "Muat Berita Lebih Lama", batch sampai 20 headline/panggilan, hasil dishare semua user | 36 jam per guid (`news_tr:<guid>`, samakan retensi `news_history`) — 1x translate seumur cache, tidak diulang | Tidak ada rate limit per-IP khusus (bukan endpoint user-triggered); dibatasi jatah harian `gemini_newstranslate` (lihat §4) |
...
| Google AI Studio (Gemini) — bucket `gemini_newstranslate` | 1000 request/hari (= 1000 PANGGILAN, bukan 1000 headline — 1 panggilan berisi batch sampai 20 headline), **TERPISAH** dari bucket `gemini` (200/hari) di atas | 10 RPM, 1.500 RPD (1 API key/project sama dengan bucket `gemini`, cuma counter kuota kita sendiri dipisah — 1000+200=1200, masih di bawah 1.500) | **HANYA** Translate NEWS (#10) — sengaja dipisah dari bucket `gemini` supaya volume translate (bisa ratusan headline/hari, tapi jauh lebih sedikit PANGGILAN berkat batch) tidak rebutan kuota Analisa Fundamental/AI Coach yang fallback ke Gemini juga |
