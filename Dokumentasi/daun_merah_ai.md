...
| 10 | **Translate NEWS ke Bahasa Indonesia** (S272, 2026-08-02 — toggle 🇮🇩/🇬🇧 di tab NEWS, field tambahan `title_id`/`desc_id`, teks Inggris asli TETAP dipertahankan utuh untuk `newscat.js`/filter TEK/push notif. Riwayat provider hari yang sama, 3 percobaan gagal terverifikasi live sebelum settle: (1) Gemini→SambaNova akun 2 murni ganti provider — circuit breaker trip berulang (22x), akun dipakai bersama 3 fitur lain; (2) balik Gemini + desain **BATCH** (1 panggilan API sampai 20 headline sekaligus, bukan 1/headline) — 429 `RESOURCE_EXHAUSTED`, kuota Gemini cuma 20 request/HARI (bukan 1.500 RPD seperti asumsi lama); (3) balik SambaNova akun 2 + batch + fix `AbortSignal` (panggilan lambat dibatalkan tegas, bukan orphaned di background) — TETAP trip lagi (72x ~1 jam pasca-deploy), akun itu sendiri terbukti tidak stabil (tes isolasi timeout 20+ detik bahkan untuk 3 headline). **Final (waktu itu): Mistral** (`mistral-small-latest`) — satu-satunya kandidat dari 3 yang dites ulang (SambaNova akun 1, Gemini flash-lite, Mistral) TANPA kontensi fitur produksi lain sama sekali, 3/3 ronde tes konsisten ~5,5 detik, akurasi terjemahan setara/lebih baik (konversi format angka EN→ID lebih presisi)). **Safeguard tambahan (2026-08-05, insiden translate macet total — lihat daun_merah.md):** deskripsi per headline dalam batch prompt dipotong maks 1200 char (item outlier seperti "MUFG FX Daily Snapshot" bisa >5000 char, bikin satu batch selalu timeout); item yang gagal >5x beruntun (`news_tr_fail:<guid>`) MENYERAH permanen (biarkan bahasa Inggris) alih-alih terus jadi kepala antrean FIFO yang memblokir headline baru di belakangnya. **PIVOT LAGI (2026-09-06, S351-352):** akun Mistral kena rate-limit 0 request/menit di sisi Mistral sendiri (perlu Pay-as-you-go aktif, bukan trial/billing seperti dugaan awal — detail `daun_merah_vendor.md`), user menolak tunggu reset 25 hari/bayar. Kandidat baru "Big Pickle" (model gratis via OpenCode Zen) DITOLAK setelah tes live: latensi 6-27 detik sangat tidak konsisten (overhead mode "thinking" yang tidak bisa dimatikan lewat `reasoning_effort`/`enable_thinking`) + `FreeUsageLimitError` cuma dari ~8 panggilan tes — berisiko menyeret timeout seluruh function `api/feeds.js`. **Provider SEKARANG: `gemini-flash-lite-latest`** — Gemini 3.x juga selalu "thinking" tapi `reasoning_effort:'low'` (pola sama market-digest.js) terbukti live efektif: batch 20 headline penuh cuma 2,6 detik, batch dengan deskripsi ~1000 char 2,3 detik, semua 100% ter-parse — lebih cepat dari baseline Mistral. Bucket kuota `gemini_newstranslate` TERPISAH dari `gemini` (16/hari, Fundamental/Jurnal) karena model beda. | Toggle bahasa (bukan tombol generate — otomatis di background) | Ya — tiap cache RSS di-refill (~50-60 detik, jendela live) DAN cron `news_translate_backfill` tiap 5 menit (arsip 36 jam PENUH, proaktif — bukan cuma pas "Muat Berita Lebih Lama" diklik) & tiap "Muat Berita Lebih Lama", batch sampai 20 headline/panggilan, hasil dishare semua user | 36 jam per guid (`news_tr:<guid>`, samakan retensi `news_history`) — 1x translate seumur cache, tidak diulang. **Cache KONTEN tambahan (2026-09-08, fix bug "translate ulang" — lihat `daun_merah.md`):** `news_tr_c:<judul+pubDate ternormalisasi>` — FinancialJuice kadang mem-broadcast ulang headline identik dengan guid BARU (kadang dalam satu payload RSS yang sama), sebelumnya tiap repost dianggap item baru & ditembak ulang ke Gemini (boros kuota, bisa kena rate limit). Sekarang repost dicek dulu lewat cache konten (0 panggilan AI kalau sudah pernah), poison-item guard juga ikut dipindah ke identitas konten yang sama supaya tidak mati fungsi kalau item bermasalah adalah jenis yang di-repost. | Tidak ada rate limit per-IP khusus (bukan endpoint user-triggered); dibatasi jatah harian `gemini_newstranslate` (lihat §4) |

**Koreksi kondisi produksi (2026-09-24).** Alias yang disebut pada uraian item #10, `gemini-flash-lite-latest`, sudah tidak tersedia di Google dan bukan lagi jalur aktif. Translate NEWS sekarang memakai nama stabil `gemini-3.5-flash-lite`, dengan `gemini-3.1-flash-lite` sebagai cadangan hanya saat primary ditolak 400/404. Ini menggantikan alias bergerak tanpa mengubah batch, cache, atau bucket `gemini_newstranslate`.
...
**[2026-09-12, S359] Kondisi aktif: AATAS_PROMPT_VERSION 11, POLICY_VERSION 57.** Gate 1 menolak driver kosong, konfirmasi identik, arah berbeda dari bias terkunci, serta inversi short-squeeze quote yang eksplisit. Event <6 jam ditentukan kalender dan menahan fill setup baru/refine. Trigger otomatis dibatasi sentuhan zona + kalender; pola/close bebas tidak dijanjikan sebagai kondisi eksekusi. Catatan efektif memakai angka final kode, laporan asli model tetap berlabel. Independensi ekonomi dua parafrasa dan kebenaran seluruh narasi belum dapat dibuktikan oleh pemeriksaan sintaks ini. Detail lengkap: professional_llm_trader/changelog.md S359. Provider/jumlah panggilan tetap.

**[2026-09-06, POLICY_EPOCHS v45, PLAN AC Tahap 3 jalan tengah] Call 2 AATAS: menu kandidat TP disaring RR>=2 dan paksaan zona terdekat dihapus.** Kandidat TP jalur auto dari api/_levels.js sekarang wajib berjarak >=2x jarak SL terdekat (parameter minRr, manual tetap 1); instruksi tp auto menyebut 1:2 (sama dengan gate Step 6). Kalimat tie-break "pilih yang lebih dekat ke Now" dihapus dari instruksi entry_zone auto, diganti tuntutan konsisten dengan technical.fib_reason AI sendiri; tidak ada tabel kedalaman fibonacci baru (keputusan user). Jalur manual publik tidak berubah. AATAS_PROMPT_VERSION 8. Detail: professional_llm_trader/changelog.md Session 353.

**[2026-09-06, POLICY_EPOCHS v44, PLAN AC Tahap 1] Call 1 AATAS sekarang menerima blok `[DATA RILIS EKONOMI]`** — angka rilis domestik kedua kaki pair dari hash `fundamental:<CUR>` (keputusan CB, CPI/core, GDP, tenaga kerja, PMI, retail, upah; actual vs forecast vs previous + tanggal + label LAMA >45 hari, maks 8 baris per currency, dipilih regex kategori bukan nama key persis). Suku bunga CB di-overlay dari cache `cb_rates_live_v2` + `cb_decisions` (cache saja, tanpa scrape); kalau cache kosong, baris rate hanya angka fallback berlabel "belum terverifikasi live" tanpa klaim keputusan. Blok yang sama masuk fact sheet Kritikus (Gate A). Call 2 dan jalur manual publik §3.2 tidak berubah. Latar: audit Session 350 lanjutan — Call 1 tidak pernah menerima data ini padahal checklist Step 0/2 menyuruh membacanya (9/9 kandidat AUD/NZD driver-nya data negara lain). Ukuran prompt Call 1 +~1,6 KB (~500 token, +10%), biaya sekitar +$0,2/bulan pada tarif v4-pro peak. `AATAS_PROMPT_VERSION` 6. Detail: `professional_llm_trader/changelog.md` §Session 351.

**[2026-08-31, POLICY_EPOCHS v37] Model dipisah per-panggilan di jalur AATAS.** Call 1 (penentu arah) = **`deepseek-v4-pro`**; Call 2 (pemilih lokasi/SL/TP dari kandidat deterministik) tetap `deepseek-v4-flash`. Dasarnya uji banding pada data live identik: keputusan trade kedua model sama persis, tapi `makro_alignment` berbeda — flash melabeli "konflik" padahal paragrafnya sendiri sudah menyelesaikan konflik itu, pro melabeli "searah" dengan penyelesaian berbasis bukti (VIX contango + HY OAS turun) dan spontan menandai jebakan price-derived. `makro_alignment` adalah input gerbang, jadi selisihnya mengubah keputusan, bukan cuma narasi. Biaya: ~10-12 panggilan/hari naik ke tarif 3x (5 pair x 2 slot + jam khusus AUD/NZD), masih jauh di bawah pagar 50/hari. Field `model` di entri setup sekarang bisa berisi gabungan `deepseek-v4-pro+deepseek-v4-flash`. Detail: `professional_llm_trader/changelog.md` §Session 337 lanjutan.
...
| Gemini `gemini-flash-lite-latest` — bucket `gemini_newstranslate` (2026-09-06, S351-352, ganti dari Mistral — lihat baris #10 §2 & `daun_merah_vendor.md`) | 800 request/hari (= 800 PANGGILAN, bukan 800 headline — 1 panggilan berisi batch sampai 20 headline), **TERPISAH** dari bucket `gemini` (16/hari, dipakai Fundamental/Jurnal via model `gemini-flash-latest` BEDA) | Model `-lite` generasi sebelumnya terdokumentasi 1.000 RPD (`daun_merah_riset.md`) — BELUM diverifikasi live di skala produksi penuh untuk alias `-latest` saat ini, cuma 3 panggilan tes sebelum deploy (semua sukses, lihat `daun_merah.md` S351-352) | **HANYA** Translate NEWS (#10). Model BEDA dari yang dipakai Fundamental/Jurnal, jadi pool kuota Google seharusnya terpisah — TAPI belum ada bukti live skala penuh, pantau `ai_budget.gemini_newstranslate` (`admin?action=health`) beberapa hari pertama |

**Koreksi Translate NEWS (2026-09-24).** Baris bucket di atas yang masih menyebut `gemini-flash-lite-latest` adalah kondisi historis sebelum alias tersebut dihentikan Google. Bucket dan angka guard tetap sama, tetapi model aktifnya sekarang `gemini-3.5-flash-lite` dengan cadangan `gemini-3.1-flash-lite` khusus respons 400/404. Tidak ada alias `-latest` dalam jalur penerjemahan.
...
|---|---|---|---|---|
| DeepSeek (API resmi, BERBAYAR) | `api.deepseek.com/chat/completions` | **Call 1 Ringkasan: `deepseek-v4-pro`** (2026-08-17, promosi — lihat `daun_merah.md` Session 317 lanjutan 2); **AATAS Call 1 (penentu arah): `deepseek-v4-pro`** (2026-08-31, POLICY_EPOCHS v37); **Call 2/3/4, AATAS Call 2, + semua fitur lain: `deepseek-v4-flash`** (thinking disabled) | **PRIMARY/SATU-SATUNYA** — Ringkasan Berita Call 1-4, Analisa AI per Pair, Pre-Entry Check, Review Posisi Virtual, Sistem Hakim Gate A | `DEEPSEEK_API_KEY` |
