...
| 10 | **Translate NEWS ke Bahasa Indonesia** (S272, 2026-08-02 — toggle 🇮🇩/🇬🇧 di tab NEWS, field tambahan `title_id`/`desc_id`, teks Inggris asli TETAP dipertahankan utuh untuk `newscat.js`/filter TEK/push notif. Riwayat provider hari yang sama, 2 percobaan gagal terverifikasi live sebelum settle: (1) Gemini→SambaNova akun 2 murni ganti provider — circuit breaker trip berulang, akun dipakai bersama 3 fitur lain; (2) balik Gemini + desain **BATCH** (1 panggilan API sampai 20 headline sekaligus, bukan 1/headline) — 429 `RESOURCE_EXHAUSTED`, kuota Gemini cuma 20 request/HARI (bukan 1.500 RPD seperti asumsi lama). **Final: SambaNova akun 2 + desain BATCH** — beban RPM jauh lebih ringan dari desain awal (1-2 panggilan/siklus, bukan 15 konkuren), plus `AbortSignal` per-panggilan dikunci ke SISA BUDGET nyata (bukan Promise.race timer terpisah) supaya panggilan lambat dibatalkan tegas, tidak orphaned di background) | Toggle bahasa (bukan tombol generate — otomatis di background) | Ya — tiap cache RSS di-refill (~50-60 detik) & tiap "Muat Berita Lebih Lama", batch sampai 20 headline/panggilan, hasil dishare semua user | 36 jam per guid (`news_tr:<guid>`, samakan retensi `news_history`) — 1x translate seumur cache, tidak diulang | Tidak ada rate limit per-IP khusus (bukan endpoint user-triggered); dibatasi jatah harian `sambanova_c1_newstranslate` (lihat §4) |
...
| SambaNova (akun-2) — bucket `sambanova_c1_newstranslate` | 1000 request/hari (= 1000 PANGGILAN, bukan 1000 headline — 1 panggilan berisi batch sampai 20 headline), **TERPISAH** dari bucket `sambanova_c1` (200/hari) di atas | ~10-20 RPM, free persisten (akun sama dengan `sambanova_c1`, cuma counter kuota kita sendiri dipisah) | **HANYA** Translate NEWS (#10) — sengaja dipisah dari bucket `sambanova_c1` supaya volume translate (bisa ratusan headline/hari, jauh lebih sedikit PANGGILAN berkat batch) tidak rebutan kuota fallback Journal/Fundamental/Call 1 Ringkasan yang berbagi akun sama |
