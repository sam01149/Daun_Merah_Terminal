...
**Hak istimewa Gemini Plus tidak menambah kuota Gemini API.** Langganan itu berlaku untuk aplikasi Gemini konsumen; API tetap memakai project Google Cloud/AI Studio dan billing API sendiri. Namun profil Google Payments yang sama dapat dipakai lagi: Indonesia ditagih dalam IDR dan Google Cloud menerima kartu debit/kredit berlogo Mastercard. Jadi BNI Taplus Muda berpeluang dipakai lewat jalur Google, terpisah dari gateway DeepSeek yang sedang menolak transaksi. Untuk masuk paid tier, dokumentasi Gemini saat ini meminta Cloud Billing ditautkan dan prepay minimal US$5 atau ekuivalen lokal. Ini bukan jaminan bank pasti menyetujui, tetapi bukan masalah yang dapat diselesaikan oleh benefit Gemini Plus. [Billing Gemini API](https://ai.google.dev/gemini-api/docs/billing), [metode pembayaran Google Cloud Indonesia](https://cloud.google.com/billing/docs/resources/currency?hl=id), [kartu yang diterima Google Cloud](https://docs.cloud.google.com/billing/docs/how-to/payment-methods?authuser=00)
...
1. User membuka/menautkan Cloud Billing personal pada project API, melakukan prepay, lalu membuat key yang dibatasi ke project/API terkait. Jangan memakai key bebas tanpa pembatasan; kuota Gemini dihitung per project, bukan per key. [Rate limit Gemini](https://ai.google.dev/gemini-api/docs/rate-limits), [pengelolaan API key](https://ai.google.dev/gemini-api/docs/api-key)
...
4. Baru jika format, latensi, tingkat kegagalan, dan seluruh gate deterministik lolos uji, migrasikan semua call DeepSeek ke model Gemini yang sama secara bertahap. Terapkan exponential backoff untuk 429/503 sesuai panduan Gemini. [Troubleshooting Gemini](https://ai.google.dev/gemini-api/docs/troubleshooting)
