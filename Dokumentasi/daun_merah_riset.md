...
- **[2026-07-22] Akselerasi Akumulasi Sampel Plan U (Skema Golden Trio 3 Pairs)**:
  ...
  - **[2026-07-26] Audit independensi Golden Trio (diskusi user — "XAU/EUR/GBP kan sama-sama ngaruh ke USD?"):** keputusan Golden Trio di atas MURNI soal kecepatan akumulasi sampel, TIDAK mempertimbangkan independensi statistik antar-pair — n≥30 per pair mengasumsikan sampel i.i.d. (syarat CLT), padahal ketiganya share kaki USD. Dibuat `scripts/analyze_pair_correlation.js` (+ `pearsonCorrelation` baru di `scripts/_stats.js`) untuk mengukur langsung, bukan menduga:
    - **Korelasi harga riil (return 1H, 60 hari, Yahoo publik, n≈1118-1422 per pasangan):** EUR/USD vs GBP/USD r=0,827 (SANGAT KUAT); XAU/USD vs EUR/USD r=0,585 (kuat); XAU/USD vs GBP/USD r=0,536 (kuat). Kontrol EUR/GBP (tidak share USD) vs EUR/USD r=0,055 dan vs XAU/USD r=-0,027 (nyaris nol) — bukti bersih bahwa korelasi tinggi Golden Trio memang disebabkan kaki USD bersama, bukan kebetulan/kebisingan data.
    - **Kesimpulan:** dugaan user TERBUKTI benar dengan angka. n≥100 gabungan dari 3 pair ini **BUKAN 100 sampel independen** — CI/p-value yang dihitung dari situ akan tampak lebih presisi dari yang sebenarnya dijamin data. Belum diputuskan tindak lanjut (opsi: ganti 1 pair ke cross non-USD, atau sekadar catat sebagai caveat di kriteria gate) — nunggu diskusi user.
    - **Cek tambahan (bias AI riil di `setup_log_auto:v1`, n=10 entri/3 slot multi-pair saat audit — MASIH SANGAT KECIL, indikatif saja):** 1/3 slot semua pair implikasikan pandangan USD yang sama, 2/3 beda arah — mengindikasikan bias arah yang di-generate AI TIDAK otomatis 100% ikut-ikutan satu sama lain walau harganya korelatif (tiap pair tetap dapat pembacaan teknikal sendiri), tapi n masih jauh terlalu kecil untuk kesimpulan apa pun. Cek ulang setelah sampel lebih besar.
    - **[2026-07-26 lanjutan] Kandidat pair pengganti — dua dimensi (kecepatan + independensi):** user awalnya menduga ganti 1 pair ke cross non-USD akan bikin n≥33 per-pair lebih lambat. Dibuat `scripts/analyze_setup_opportunity_rate.js` (proxy OBJEKTIF pakai `_classifyStructure`+`_confluenceZones`, fungsi murni yang SAMA dipakai produksi, nol AI call) untuk mengukur seberapa sering suatu pair historisnya punya struktur H4 jelas + zona konfluensi layak (proxy "AI kemungkinan tidak menolak bikin setup") — dikombinasikan dengan `analyze_pair_correlation.js` buat cek independensi tiap kandidat. 4 kandidat dites (EUR/GBP, AUD/JPY, AUD/NZD, CHF/JPY) vs rata-rata 3 pair Golden Trio (peluang setup ~58%):
      | Kandidat | Peluang setup (n=99 titik/60 hari) | vs rata-rata Golden Trio | Korelasi rata-rata \|r\| ke 3 pair existing |
      |---|---|---|---|
      | AUD/NZD  | 68% | +10 | **0,10 (terendah — terbaik)** |
      | AUD/JPY  | **71%** | **+13 (tercepat)** | 0,45 |
      | EUR/GBP  | 54% | -4 | 0,19 |
      | CHF/JPY  | 63% | +5 | 0,48 (terkorelatif — kalah di dua dimensi) |
      - **Temuan kunci:** dugaan awal user (ganti pair = pasti lebih lambat) TERBUKTI TIDAK SELALU BENAR — tergantung kandidat spesifiknya (EUR/GBP memang sedikit lebih lambat, tapi AUD/JPY & AUD/NZD malah lebih cepat dari Golden Trio saat ini).
      - **AUD/JPY BUKAN fix independensi yang bersih** — meski tidak share USD, dia tetap korelasi sedang (bukan nol) ke ketiga pair karena sama-sama proxy rezim "risk sentiment" global (JPY safe-haven vs AUD/EUR/GBP/XAU risk-currency).
      - **CHF/JPY gagal total** — hipotesis "dua safe-haven saling menetralkan" (pola sama EUR/GBP) TIDAK terbukti; SNB vs BOJ punya pendorong independen sendiri-sendiri, jadi pair ini malah kalah di dua dimensi sekaligus.
      - **AUD/NZD (Antipodean cross) adalah kandidat TERBAIK** — dua mata uang komoditas dengan eksposur risk-sentiment mirip, silangnya saling menetralkan faktor bersama (pola sama EUR/GBP) SEKALIGUS opportunity rate-nya tinggi (hampir menyamai AUD/JPY). Satu-satunya kandidat kuat di kedua dimensi tanpa trade-off berarti.
      - **Batasan metode:** proxy opportunity-rate ini batas ATAS (tidak memodelkan penolakan AI karena konflik makro), dan korelasi harga BUKAN jaminan korelasi outcome win/loss (belum ada data riil cukup besar buat verifikasi langsung). Belum diputuskan eksekusi — nunggu keputusan user, dan kalau dieksekusi perlu tambah mapping symbol baru di `AUTO_ENTRY_SYMBOL_MAP` (`vps/daemon.js`) + cek ketersediaan simbol Deriv buat live streaming.
    - **[2026-07-26 keputusan] Kandidat final: 4 pair — EUR/USD, XAU/USD, AUD/NZD, EUR/GBP (GBP/USD dibuang, AUD/NZD + EUR/GBP masuk).** GBP/USD dibuang karena paling redundan (r=0,827 ke EUR/USD, korelasi tertinggi di seluruh set — kontribusi informasi baru paling kecil). EUR/USD & XAU/USD dipertahankan (anchor major + kelas aset beda yang tetap relevan ke fitur publik lain). AUD/NZD masuk karena menang di DUA dimensi (independensi r≈0,10 terendah, opportunity rate 68%). EUR/GBP ikut ditambahkan (bukan menggantikan, menambah ke-4) karena dicek korelasinya nyaris nol ke SEMUA 3 anggota lain (vs EUR/USD r=0,055, vs XAU/USD r=-0,027, vs AUD/NZD r=0,028) — aman ditambahkan tanpa menciptakan korelasi baru. Sebaliknya AUD/JPY & CHF/JPY SENGAJA tidak diikutkan ke set final — keduanya korelasi sedang (0,3-0,5) ke anggota lain, menambahkannya balik mengencerkan perbaikan independensi yang baru didapat.
      - **PENTING — kriteria "cukup data" WAJIB direvisi seiring jumlah pair berubah, bukan angka 3 yang sakral:** Golden Trio (3 pair, 6 call/hari) mendesain n≥100 global dan n≥30/pair supaya nyampe BARENGAN di ~16-17 hari — itu kebetulan hasil optimasi utk SET PAIR waktu itu (yang korelatif), bukan aturan universal "harus 3 pair". Kalau stopping rule TETAP "berhenti begitu n global tembus 100" tanpa direvisi, 4 pair (8 call/hari) akan berhenti di ~12,5 hari dengan n rata-rata cuma ~25/pair — DI BAWAH ambang CLT 30, secara harfiah melanggar syarat yang sama yang dulu menolak skema 6-pair.
      - **Fix:** kriteria berhenti Plan U untuk skema 4-pair ini WAJIB "yang PALING LAMBAT dari: (a) n global ≥100, ATAU (b) SEMUA 4 pair individual sudah ≥30" — bukan cuma (a) saja. Dengan itu, kebutuhan total naik ke 4×30=120 setup, di 8 call/hari ≈ **~15 hari** — hampir sama (malah sedikit lebih cepat) dari ~16-17 hari skema 3-pair lama. Jadi TIDAK berlawanan dengan riset Golden Trio kalau kriteria ini diikutkan; JADI berlawanan (dan merusak validitas n≥30/pair) kalau kriteria lama dipakai apa adanya tanpa revisi.
      - **Status:** kandidat final, BELUM dieksekusi ke kode. Checklist eksekusi (kalau lanjut): (1) tambah `AUD/NZD` + `EUR/GBP` ke `AUTO_ENTRY_SYMBOL_MAP`/`AUTO_ENTRY_PAIRS` (`vps/daemon.js`), buang `frxGBPUSD`; (2) verifikasi simbol Deriv (`frxAUDNZD`, `frxEURGBP`) tersedia utk live streaming candle (baru simbol Yahoo yang terverifikasi ada); (3) putuskan nasib 10 entri lama `setup_log_auto:v1` (arsip terpisah vs dianggap tidak relevan lagi krn populasi pair berubah); (4) update gate n≥100/n≥30 di kode/dashboard (`dev-auto-entry.html`, `setup_stats`) supaya baca "semua pair ≥30" bukan cuma global, sesuai kriteria revisi di atas.
