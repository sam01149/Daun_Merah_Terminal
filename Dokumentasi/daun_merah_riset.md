...
### [2026-09-16] Gemini berbayar sebagai satu pengganti DeepSeek — kelayakan dan prasyarat auto-entry virtual

**Gemini API memang mempunyai free tier; Gemini Plus tidak menambah kuota API itu.** Free tier memberi akses model tertentu dengan token input/output gratis sehingga cocok untuk uji diagnostik. Langganan Plus berlaku untuk aplikasi Gemini konsumen, bukan tambahan limit API. Kuota bergantung pada model dan project (bukan API key), dan limit aktif harus dilihat di AI Studio. Proyek ini pernah mengukur sekitar 20 request/hari pada alias flash lama; itu bukan batas universal untuk semua model atau project, maka model Flash terbaru perlu diuji dulu tanpa biaya sebelum menyimpulkan free tier tidak cukup. [Pricing Gemini API](https://ai.google.dev/gemini-api/docs/pricing), [rate limit Gemini](https://ai.google.dev/gemini-api/docs/rate-limits)

Jika hasil uji free tier tidak cukup, baru profil Google Payments yang sama dapat dicoba untuk paid tier: Indonesia ditagih dalam IDR dan Google Cloud menerima kartu debit/kredit berlogo Mastercard. Paid tier membutuhkan Cloud Billing dan, untuk akun prepay, minimal US$5 atau ekuivalen lokal. Ini jalur berbeda dari gateway DeepSeek dan bukan jaminan bank menyetujui transaksi. [Billing Gemini API](https://ai.google.dev/gemini-api/docs/billing), [metode pembayaran Google Cloud Indonesia](https://cloud.google.com/billing/docs/resources/currency?hl=id), [kartu yang diterima Google Cloud](https://docs.cloud.google.com/billing/docs/how-to/payment-methods?authuser=00)

**Penggantian penuh ke Gemini secara teknis mungkin, tetapi belum boleh dipromosikan langsung.** Kode sudah memakai `GEMINI_API_KEY` untuk Fundamental, Jurnal, Terjemahan, fallback Ringkasan Call 1/2, serta jalur uji terisolasi Ringkasan Call 1-3. Yang belum ada adalah jalur uji Gemini untuk seluruh auto-entry virtual: dua call AATAS, Critic/Gate A, pre-entry check, dan position review. Semua jalur ini memakai kontrak JSON ketat dan validator deterministik; Gemini dapat memakai structured output, tetapi structured output hanya menjamin bentuk JSON, bukan kebenaran harga, level, atau alasan. Validator aplikasi tetap wajib dipertahankan. [Structured output Gemini](https://ai.google.dev/gemini-api/docs/structured-output)

**Urutan aman sebelum migrasi kode:**

1. Cek rate limit aktif model Flash di AI Studio pada project yang sudah ada. Selama cukup untuk uji, gunakan free tier dulu; jangan menautkan billing hanya karena memiliki Gemini Plus. [Rate limit Gemini](https://ai.google.dev/gemini-api/docs/rate-limits)
2. Kode menambahkan mode diagnostik Gemini khusus developer untuk **auto-entry virtual saja**, tanpa mengubah entry nyata atau cache produksi. Hasilnya dibandingkan dengan DeepSeek pada sejumlah kondisi pasar dan respons gagal sebelum provider utama diganti.
3. Budget, rate-limit, dan circuit breaker Gemini dipisah antara fitur publik/manual dan auto-entry eksperimental. Satu provider tidak boleh berarti satu kuota bersama yang bisa dihabiskan satu fitur sampai mengganggu fitur lain.
4. Bila free tier terbukti cukup, lanjutkan migrasi bertahap ke model Gemini yang sama. Bila tidak, user dapat memilih paid tier dulu. Terapkan exponential backoff untuk 429/503 sesuai panduan Gemini. [Troubleshooting Gemini](https://ai.google.dev/gemini-api/docs/troubleshooting)

**Batas penggunaan:** tetap hanya developer virtual dan hasilnya harus dapat diaudit. Jika suatu saat dihubungkan ke broker/eksekusi nyata, kebijakan provider dan kepatuhan harus dinilai ulang sebelum diaktifkan. Belum ada perubahan kode, secret, billing, atau provider dari riset ini.

**Trade-off yang terkonfirmasi dari fitur Fundamental (cek ulang 2026-09-16):** free tier memang dapat diuji, tetapi bukan pengganti penuh tanpa risiko. Alias `gemini-flash-latest` yang dipakai Fundamental pernah berpindah generasi lalu limit nyata proyek menjadi 20 request/hari; dokumentasi Google menegaskan alias `latest` bisa di-hot-swap, sedangkan aplikasi produksi sebaiknya memakai model stable yang spesifik. Fundamental sendiri hemat (maksimal 4 generasi cache/hari, atau hingga 8 panggilan bila retry), tetapi ia berbagi guard Gemini 16/hari dengan AI Coach Jurnal. Beban auto-entry jauh di atas itu: sekitar 10 kandidat/hari, masing-masing Call 1 dan Call 2, dengan sebagian juga memanggil Critic — estimasi normalnya sudah sekitar 20-26 call/hari sebelum fitur publik. Jadi free tier dapat membuktikan kualitas/format pada jalur diagnostik kecil, tetapi **tidak boleh dijanjikan menanggung seluruh DeepSeek** sebelum limit model stable aktif di AI Studio dibaca dan diukur. [Pola nama model Gemini](https://ai.google.dev/gemini-api/docs/models), [rate limit Gemini](https://ai.google.dev/gemini-api/docs/rate-limits)

Masalah Fundamental yang nyata juga sudah ditangani sebagian, bukan hilang: Gemini pernah memberi 503 transient; kode kini retry sekali setelah 2 detik, tetapi dua kegagalan tetap mengembalikan HTTP 500. Output panjang juga pernah terpotong karena token reasoning; `reasoning_effort:'low'`, `max_tokens:3500`, dan pembersih markdown server sudah menjadi mitigasi, bukan bukti output akan selalu patuh format. Untuk migrasi jalur keputusan, JSON/level harus terus divalidasi kode dan alias harus dipin ke model stable setelah lolos tes; `latest` tidak cukup deterministik sebagai kontrak produksi. Google sendiri menyarankan exponential backoff untuk 429/503. Free tier juga menyatakan prompt/respons dapat dipakai untuk peningkatan produk, sehingga tidak tepat untuk data jurnal pribadi tanpa persetujuan user. [Troubleshooting Gemini](https://ai.google.dev/gemini-api/docs/troubleshooting), [pricing dan data free tier](https://ai.google.dev/gemini-api/docs/pricing)
...
- **Tool/model forecasting baru tidak otomatis membuka kembali proyek yang di-stop karena tidak ada sinyal.** User tanya (2026-09-02) apakah TimesFM (Google Research, foundation model transformer untuk time-series forecasting) memberi "harapan" untuk proyek NFP causal (`project_delay/machine learning/ml/NFP_PROYEK`, STOP final karena kill-gate gagal 0/25 — lihat memory `nfp-causal-research-framework`). Kesimpulan: TIDAK qualify sebagai "metode berbeda fundamental" yang jadi syarat buka-ulang. Alasan: kegagalan proyek itu adalah masalah *tidak ada sinyal prediktif* yang lolos uji permutasi (konsisten literatur Klein 2022 — konsensus pasar sudah efisien), bukan masalah *model forecasting kurang canggih*. TimesFM juga mismatch bentuk soal (autoregressive continuation vs uji hubungan sebab-akibat lintas-indikator dengan permutation test) dan mismatch skala data (pretrained data frekuensi tinggi vs panel bulanan n~342). **Implikasi:** saat mengevaluasi tool/model baru terhadap proyek yang sudah di-stop, cek dulu apakah alasan stop itu soal kapabilitas model atau soal ketiadaan sinyal di data — kalau yang kedua, model baru secanggih apa pun kemungkinan besar tidak menolong.
  **Follow-up sama hari — proyek BTC (`project_delay/machine learning/ml/`, dua sub-target beda nasib):** (1) *price-direction* — dead end identik pola NFP (AUC 0.528, ACF/PACF ~0 di semua lag → tidak ada struktur sama sekali untuk digali model apa pun); TimesFM tidak relevan di sini. (2) *vol-regime classification* — ini justru SUKSES & sudah produksi (AUC 0.63, `target_vol_regime_6`), TAPI 4 percobaan tambahan (DVOL, GARCH, fear_greed, VIX) semua gagal naikkan AUC karena redundan dengan fitur rolling-vol yang sudah ada (GARCH korelasi 0.956 dengan fitur existing). TimesFM di sini architecturally lebih relevan (quantile-spread ≈ forecast volatilitas langsung, bukan fitur turunan), tapi risiko tinggi jatuh ke jebakan sama seperti GARCH (rentang quantile-nya kemungkinan cuma mengikuti volatilitas historis terbaru = info yang sudah tertangkap). Dicatat sebagai "rejected idea belum diuji" di `project_delay/machine learning/ml/STATUS.md` — kalau mau dicoba, wajib rigor sama (walk-forward CV + permutation test langsung ke delta-nya) seperti 4 kandidat sebelumnya.
