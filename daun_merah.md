...
> **Last updated:** 2026-06-22 (session 75 — Tab TEK: ganti dropdown pilihan pair dari `<select>` native 8 major jadi combobox searchable+scrollable (input pencarian di atas, list max-height 260px overflow-y:auto), dipicu permintaan user "tambahin semua kombinasi pair tapi bisa di search + scroll, soalnya kalau enggak pasti kebanyakan". Pair list diperluas dari 8 major jadi 29 (7 major + XAU/USD + 21 cross pair, digenerate programatis dari 8 currency: EUR/GBP/AUD/NZD/USD/CAD/CHF/JPY via nested loop, bukan ditulis manual). Refactor `TEK_PAIR_KEYWORDS` (keyword matching utk "Berita Terkait") dari hardcoded per-pair list jadi composable: `TEK_CUR_KEYWORDS` per currency + `TEK_SHARED_KEYWORDS` (china/geopolit/tariff/dll, sengaja dipakai di semua pair sebagai general risk-sentiment driver) → tiap pair = union dari kedua leg currency-nya + shared, jadi EUR/GBP otomatis dapat keyword EUR+GBP tanpa ditulis manual (dikonfirmasi user: "berlakukan utk semua kombinasi pair" bukan cuma contoh EUR/GBP). `TEK_TV_SYM` (TradingView), `TEK_YAHOO_SYM` (Yahoo Finance utk RSI/SMA), dan `PAIR_CURS` (kalender currency utk event strip) ikut digenerate dari list pair yang sama. **Test:** karena sandbox tidak ada browser/playwright/chromium-cli, verifikasi logic dilakukan dengan extract fungsi asli dari `index.html` + jalankan di Node lewat DOM stub minimal (bukan rewrite manual) — confirmed: search "jpy" mengembalikan 7 pair JPY yang benar, `selectTekPair('EURJPY')` update label + tutup dropdown + keyword union benar (EUR+GBP+JPY shared terms), total 29 pair unik, simbol TV/Yahoo/PAIR_CURS XAU/USD tetap pakai special-case lama (`FOREXCOM:XAUUSD`, `GC=F`, `['USD']`). **Fix kedua di sesi ini:** tab ANALISA AI tidak punya entry point di mobile (cuma ada di top-nav desktop `.nav-views`, yang di-hide total di mobile; tidak ada tombol di `#botNav` maupun di menu "Lainnya"/`DRAWER_ITEMS`) — user cuma bisa akses lewat chip tersembunyi di Dashboard yang perlu di-scroll. Fix: tambah entry `{view:'analisa', label:'Analisa AI', ...}` di awal `DRAWER_ITEMS`, otomatis muncul di drawer "Lainnya" karena rendering & click-handler drawer generic dari array (klik → cari `#navViews .nvtab[data-view="analisa"]` lalu trigger click programatis).)
