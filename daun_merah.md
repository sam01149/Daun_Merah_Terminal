...
> **Last updated:** 2026-06-22 (session 76 — Fix bug link salah di tab ARTIKEL sumber MTM (Marc to Market): user lapor isi artikel yang dibuka "pusing dibaca" — ternyata link yang disimpan bukan halaman artikel (`marctomarket.com/...`) tapi raw Atom XML endpoint Blogger (`blogger.com/feeds/.../posts/default/<id>`). Root cause: `parseCBRSSItems()` di `api/feeds.js` ekstrak link dengan ambil `<link>` pertama yang ketemu tanpa cek atribut `rel` — utk feed Atom (Blogger/FeedBurner, dipakai MTM), entry punya 3 `<link>` (`rel=edit`, `rel=self`, `rel=alternate`) dan urutan di XML kebetulan `edit` duluan, jadi yang keambil link mentah bukan link baca. Fix: tambah helper `pickAtomLink()` yang scan semua `<link>` tag dalam satu entry, prioritaskan `rel="alternate"` (halaman HTML yang readable), fallback ke href pertama kalau tidak ada `rel=alternate` (utk source lain yang strukturnya beda, backward compatible). **Test:** dicoba dengan XML Atom yang sama persis dengan yang dilaporkan user (3 `<link>` tag, urutan edit→self→alternate) — hasil link sekarang benar resolve ke `https://www.marctomarket.com/2026/06/week-ahead-...`. Cache `research_cache` (TTL 6h) belum di-flush manual karena butuh `CRON_SECRET` produksi; akan otomatis refresh dalam ≤6 jam atau di fetch berikutnya.)
