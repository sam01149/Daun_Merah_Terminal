...
> **Last updated:** 2026-06-22 (session 78 — Fix mobile: tombol "Muat Korelasi Cross-Asset" di tab TEK ketutupan bottom nav bar, dilaporkan user via screenshot. Root cause: `#teknikalPanel` (dan `#risetPanel`) override padding `.feed-scroll` jadi 0 inline (krn punya layout header/chart sendiri), tapi cuma `risetPanel` yang dikompensasi — elemen `.riset-list` di dalamnya punya `padding-bottom: calc(60px + safe-area)` sendiri (baris 150), sedangkan `.tek-mtf-section` (section terakhir di TEK, isinya note/Berita Terkait/option expiries/correlations) tidak. Akibatnya konten paling bawah TEK mentok pas di belakang bottom nav yang `position:fixed`. Fix: tambah rule mobile `.tek-mtf-section { padding-bottom: calc(60px + env(safe-area-inset-bottom)); }` sejajar dengan `.feed-scroll`/`.ck-sections` yang sudah ada. **Audit menyeluruh** (diminta user "cek juga fitur lain"): grep semua `class="feed-scroll"` (11 panel) — cuma `risetPanel` & `teknikalPanel` yang override padding ke 0, sisanya pakai default class jadi otomatis kebagian padding-bottom mobile. `checklistPanel` sudah dihandle terpisah via `.ck-sections`. `dashboardPanel` di mobile `display:none` total (desktop-only via `@media min-width:1024px`), jadi tidak relevan. Cek juga semua `position:fixed` lain di file — semuanya modal/overlay full-screen atau toast yang sudah punya jarak aman (`bottom:72px`), tidak ada yang collision dengan bottom nav selain kasus TEK ini.)> **Branch:** main — semua perubahan deployed ke production
