...
> **Last updated:** 2026-06-20 (session 73 — Audit kebersihan codebase: hapus file sampah 0-byte (`cd`, `git` sisa migrasi Netlify→Vercel) dan `api/package.json` (manifest Netlify lama, sudah tidak dipakai sejak migrasi Vercel). Buang 2 export tidak terpakai (`CONTRACT_CODE` di `scripts/lib/cot-bitcoin.js`, `SYMBOL` di `scripts/lib/btc-sources.js`) — diverifikasi tidak ada importer, `node --check` lolos. **Temuan keamanan:** `environtment.txt` ternyata berisi value secret asli (`SCRAPER_API_KEY`, `CRON_SECRET`) yang ter-commit ke git sejak session sebelumnya, padahal komentarnya sendiri melarang ini. Di-untrack dari git (`git rm --cached`, ditambahkan ke `.gitignore`) — file tetap ada lokal dengan isi aslinya, tapi tidak lagi ke depan ter-push. Commit lama yang sudah berisi secret tetap ada di history GitHub (user pilih tidak rewrite history) — **rotate `CRON_SECRET` dan `SCRAPER_API_KEY` masih perlu dilakukan manual oleh user untuk benar-benar aman.** Juga di-untrack: `daun_merah_handoff.md` (stale sejak April, sudah lama digantikan `daun_merah.md`) dan `daun_merah_plan.md` (backlog aktif, tapi sudah lama terdaftar di `.gitignore` sebagai local-only, sekarang konsisten dengan intent-nya). Tidak ada perubahan fungsional/behavior produksi di session ini — murni housekeeping repo.)
