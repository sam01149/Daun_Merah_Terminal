...
> **Last updated:** 2026-06-22 (session 77 — Fix "Berita Terkait" tab TEK ternyata tidak benar2 pair-specific untuk cross pair: user komplain "kamu ga buat" padahal sudah ada `TEK_PAIR_KEYWORDS` dari sesi sebelumnya. Diverifikasi dengan fetch live FinancialJuice feed (`/api/feeds?type=rss`) + jalankan filter logic asli secara isolated: untuk EUR/GBP, CAD/CHF, AUD/NZD, GBP/CHF, top-10 hasil hampir IDENTIK satu sama lain — semua didominasi headline generik China/Russia/Iran karena `TEK_SHARED_KEYWORDS` match begitu banyak item sehingga headline yang benar2 spesifik currency (ada — terverifikasi "Sterling slips...", "French foreign minister..." utk EUR/GBP) terkubur di luar limit 10 item. Fix: pisah `TEK_PAIR_KEYWORDS` (kembali ke keyword spesifik 2 leg currency saja, tanpa shared) dari `TEK_SHARED_KEYWORDS`; di `renderTekNews()`, filter dua tahap — kumpulkan match spesifik dulu, baru isi sisa slot (kalau ada) dengan match shared/generik. Jadi headline pair-specific selalu nongol duluan, shared/geopolitik cuma fallback pengisi. **Test:** re-run filter logic yang sama dengan live feed snapshot — EUR/GBP sekarang nongolin Sterling + French FM + Italy di 3-4 slot teratas baru disusul generic; XAUUSD tetap dominan specific (31 match) krn keyword XAU sendiri sudah luas.)
