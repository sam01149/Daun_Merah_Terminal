...
> **Last updated:** 2026-06-19 (session 72 — BTC: EDA khusus target volatility-regime, uji GARCH(1,1) + fear_greed extremity + VIX (cross-asset), mitigasi multikolinearitas pada fitur, dan regresi besaran volatilitas (vs klasifikasi biner yang sudah ada). Hasil jujur: SEMUA kandidat fitur baru (GARCH, sentiment extremity, VIX) TIDAK menambah AUC secara signifikan — GARCH ternyata 0.956 berkorelasi dengan fitur yang sudah ada (redundan), VIX delta-nya gagal permutation test langsung (p=0.300). Regresi besaran volatilitas juga GAGAL (lebih buruk & tidak stabil dari klasifikasi) — target-nya (forward_vol, std dari cuma 6 observasi) terlalu noisy untuk prediksi nilai eksak. AUC volatility-regime klasifikasi tetap ~0.63 setelah fitur dipangkas dari 25→19 (tidak ada cost). Bersama hasil DVOL session 71, ini menjawab TUNTAS permintaan user untuk push ke AUC 70% — empat kandidat berbeda semua dites dengan rigor sama, semua gagal; riset BTC ML sekarang benar-benar mentok tanpa input/data baru dari user. Output yang bisa dipakai: classifier biner `target_vol_regime_6`, bukan forecast magnitude)
